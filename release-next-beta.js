...
async function run() {
  try {
    ...
    if (!skipVersion) {
      ...
      const out = await spawnify(`git status --porcelain`)
      if (out.stdout) {
        throw new Error(`Has unsaved git changes: ${out.stdout}`)
      }

      console.log('versioning...')
      const versionOut = await spawnify(`yarn lerna vesion ${version}`)
      if (versionOut.stderr) {
        throw new Error(versionOut.stderr)
      }
    }
    ...
    const { confirmCommit } = await prompts({
      type: 'confirm',
      name: 'confirmCommit',
      message: 'All good, ready to commit, tag and push?',
    })
    ...
    await spawnify(`git add -A && git commit -m "${versionString}"`)

    console.log(`tagging...`)
    await spawnify(`git tag "${versionString}"`)

    console.log(`pushing...`)
    await spawnify(`git push origin head`)

    console.log(`pushing tag...`)
    await spawnify(`git push origin ${versionString}`)
