...
// which workflow the trusted publisher will accept a token from. the language
// server publishes from its own lane, so its packages need trust pointing there
// rather than at the main release
const workflowFlag = process.argv.indexOf('--workflow')
const workflowFile = workflowFlag === -1 ? 'release.yml' : process.argv[workflowFlag + 1]
// an explicit list, for packages that are not workspace dirs here: the eight LSP
// platform leaves are generated by CI, and the umbrella carries skipPublish so the
// main release lane never cuts it
const packagesFlag = process.argv.indexOf('--packages')
const explicitPackages =
  packagesFlag === -1
    ? null
    : process.argv[packagesFlag + 1]
        .split(',')
        .map((name) => name.trim())
        .filter(Boolean)
...
async function main(): Promise<void> {
  ...
  const packages = explicitPackages
    ? [...explicitPackages].filter((name) => !only || name === only).sort()
    : // mirrors the filter in scripts/release.ts so the plan matches what CI publishes
      (await discoverPublicWorkspacePackages(root))
