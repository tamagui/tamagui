import { VenetianMask } from '@tamagui/lucide-icons'
...
import type { BuildPalette, BuildTheme, BuildThemeMask } from '../../types'
...
import { BuildThemeItemMask } from './BuildThemeItemMask'
  ...
  return (
    <XStack ai="center" space>
      <AddDropdown open={show} onOpenChange={setShow}>
        <YGroup>
          <AddDropdown.Title>Mask themes</AddDropdown.Title>

          {maskThemePresets.map((theme) => (
            <AddDropdown.Item
              iconAfter={VenetianMask}
              size="$3"
              key={theme.name}
              onPress={() => {
...
const maskThemePresets: Omit<BuildThemeMask, 'id'>[] = [
  {
    type: 'mask',
    name: 'dim',
    masks: [
