...
import { IS_TAURI, STUDIO_TITLEBAR_HEIGHT } from '~/src/constants'
import { useDemoProps } from '../hooks/useDemoProps'
import { StudioPaletteBar } from '../StudioPaletteBar'
import { optionValues } from './constants/demoOptions'
import { useIsScrolled } from './helpers/useIsScrolled'
...
export const StudioPreviewComponentsBar = memo(({ scrollView }: { scrollView: any }) => {
  const isScrolled = useIsScrolled(scrollView)

  return (
    <XStack
      data-tauri-drag-region
      className="all ease-in ms300"
      y={(isScrolled ? -13 : -5) + (IS_TAURI ? -2 : 0)}
      $platform-web={{
        position: 'sticky' as any,
        top: 10 + STUDIO_TITLEBAR_HEIGHT,
        marginBottom: -STUDIO_TITLEBAR_HEIGHT - 15,
        zIndex: 1000,
      }}
    >
      <XStack
