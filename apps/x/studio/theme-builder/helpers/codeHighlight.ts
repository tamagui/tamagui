// duplicate file from site's code highlight - TODO: move this into a shared util
...
const applyMultilineFix = (ast) => {
  ...
  // HTML to AST
  const hast = unified()
    .use(parse['default'] || parse, { emitParseErrors: true, fragment: true })
    .parse(html)
...
export default function (content: string) {
  const ast = refractor.highlight(content, 'tsx')

  const formattedAst = applyMultilineFix(ast)
  const numbered = lineNumberify(formattedAst).nodes
  const wrapped = wrapLines(numbered, '0')
  return toHtml(wrapped)
}
