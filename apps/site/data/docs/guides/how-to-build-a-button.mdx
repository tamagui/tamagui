...
Much more than just some text in a box, a button involves icons, loading indicators, various colors, importances and sizes, and always a delicate balancing act involving internal spacing and sub-component styling, all ideally delivered in a simple API that still accounts for styles across many states: base, hovered, focused, pressed and disabled.

Tamagui makes all of this possible, and even easy as of version `1.28`, with the introduction of `createStyledContext`, a helper which allowed us remove a large amounts of brittle code internally in favor of a much simpler, typed API that we've been iterating on for a while now. It should have the same effect for your own codebases.

We think it makes designing composable components APIs a lot better. What are composable components? It's a term of art coined by [Radix](https://www.radix-ui.com/) (as far as we know) for when you export an API that involves multiple React components that are meant to render together, sharing state and/or styling (eg, a `<Button />` that also has `<Button.Text />` and `<Button.Icon />`.

We made this guide to show you why they make sense, how to design your own, and how `createStyledContext` helps us achieve much greater simplicity in implementing them.
