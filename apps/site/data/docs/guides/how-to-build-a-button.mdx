...
That's because merging things like themes, animations, variants, media queries, and psuedo queries is quite complex, and if Tamagui doesn't know that there is a functional component in between some styled components it can merge lead to unexpected behavior. It _still works, technically_, but you'd see some unexpected behavior.
...
This in fact it returns a `React.Context` just like `createContext` does, so you can use it with `useContext` later on just as you'd expect.
...
Instead of going through `styled`, this component will be just a plain functional component. It will still work nicely with the Tamagui theme system and size, and show how you can easily use Tamagui alongside an external React component.
...
If you are working on a smaller app, one that shows only a few buttons at a time, and with a very small set of variations, then abstracting this into a single functional Button component to save some verbosity is totally fine.

But we'd that encourage you to give the composable component API a try, and build your own button using the code from the beginning of this guide.

We look forward to never having to carefully add a forwardRef, redo how we attach types, thread context manually down, and then still accidently break context value memoization again!
