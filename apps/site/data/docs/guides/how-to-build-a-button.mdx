...
It's not just some text in a box. Beyond varying colors and a few levels of importance to indicate, you'll often have multiple sizes, the ability to display icons (before or after text), a loading indicator, disabled state, and... well, some delicate internal spacing and styling that must work across base, focused, hovered and pressed states.

And of course even with all of that you need somehow balance provide both a simple API up front, while allowing consumers to re-style or attach events and accessibility attributes across every sub-component.

Luckily, Tamagui makes it easy. Or at least it as of version `1.28` it does.

We've landed a new construct that's been much needed, `createStyledContext`, which replaces large amounts of brittle code within our codebase with a much simpler pattern that fits like a final piece of the puzzle in our quest to make "composable components", ie, multiple components that work together to share styles, easier to author.
