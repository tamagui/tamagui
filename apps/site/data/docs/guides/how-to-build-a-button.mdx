...
More than just text in a box, a button involves icons, themes, variants and sizes, loading indicators and several pseudo states, and a delicate balance between its sub-components' position, spacing and style. All of this ideally delivered in a simple API that still allows users customize every piece.

Tamagui helps make this possible, and maybe even easy, as of version `1.28` with the introduction of `createStyledContext`.

We think it makes designing composable components a joy, so we made this guide to show why they are useful, how to design them, and how `createStyledContext` helps make authoring them a breeze.
