...
More than just text in a box, a button involves icons, themes, importances and sizes, loading indicators and several pseudo states, and a delicate balance between its sub-components' position, spacing and style. All of this ideally delivered in a simple API that still allows users customize every piece.

Tamagui helps make this possible (and maybe even easy as of version `1.28`). We've introduced `createStyledContext`, a helper which has been dogfooded internally to fix many brittle and verbose areas of our code.

We think it makes designing composable components APIs a joy and made this guide to show why they make sense, how to design them, and how `createStyledContext` helps achieve simplicity without sacrificing control.

What are composable components?

It's a term of art coined by [Radix](https://www.radix-ui.com/) (as far as we know) for when you export an API that involves multiple components that are meant to render together and share state and/or style, eg, a `<Button />` that also has `<Button.Text />` and `<Button.Icon />`.
