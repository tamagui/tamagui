...
Tamagui allows for building a button with "composable components," a concept popularized by [Radix](https://www.radix-ui.com/). With the introduction of `createStyledContext` in version 1.28 it's become especially easy, so we wrote this guide to show off building more advanced composable component APIs.
...
Keep in mind that this is an advanced use case. Usually you can just grab Tamagui components off the shelf and use them with inline styles. But sometimes you want to design a flexible API for your users, which this guide covers.

<Notice>
  This guide is more of a primer on designing composable components teaching many advanced
  concepts that aren't necessary within most apps. If you just a simple, ready to use
  Button, you can use the [Tamagui Button](/docs/components/button) itself directly.
</Notice>
