...
<IntroParagraph>
  Deceptively simple, designing a robust yet flexible button API in any UI framework can
  be a surprisingly difficult task to pull off.
</IntroParagraph>

More than just text in a box, a button needs icons, themes, variants, sizes, loading indicators, several interactive states, and a delicate balance between sub-components' positions, spacing and style. All of this delivered in a simple package that still allows complete customization.

Tamagui allows for building a button with "composable components," a concept popularized by [Radix](https://www.radix-ui.com/). With the introduction of `createStyledContext` in version `1.28` its become especially easy, so we wrote this guide.

What are composable components? Put simply, it's when you have, say, a `<Button />` that accepts children `<Button.Text />` and `<Button.Icon />`. And why do they exist? We'll explain that and more in this guide.

<Notice>
  Note: you can just use the [Tamagui Button](/docs/components/button) out of the box.
  This guide is for those who want extra customization, or a reference to how to design
  composable components in general.
</Notice>
