...
Luckily, Tamagui makes this much easier as of version `1.28`, with the introduction of `createStyledContext`, which removes large amounts of brittle code in our codebase and should do the same for yours.

It makes our "composable components" (ie, components that are meant to work together to share styles) feel more natural both as an UI kit maintainer and user. So we've created this guide to show how you can use it, and why we've designed it the way we have.
