...
It's a term of art coined by [Radix](https://www.radix-ui.com/) (as far as we know) for when you export an API where you render multiple components that are meant to work together, eg, a `<Button />` that also has `<Button.Text />` and `<Button.Icon />`.
