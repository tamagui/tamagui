...
More than just text in a box, a button needs icons, themes, variants, sizes, loading indicators, several interactive states, and a delicate balance between sub-components' positions, spacing and style. All of this delivered in a simple API that still allows complete customization.

Tamagui makes building a button a breeze through "composable components" (a term of art coined by [Radix](https://www.radix-ui.com/)), especially with the introduction of `createStyledContext` as of version `1.28`.

So we wrote this guide to make sense of why and how they work.

What are composable components? Simply multiple components that work together, for example a `<Button />` that also has `<Button.Text />` and `<Button.Icon />`.
