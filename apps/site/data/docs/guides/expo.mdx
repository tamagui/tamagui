...
If you'd like an example of an Expo web app albeit not kept up to date, [check out this repo](https://github.com/LunatiqueCoder/expotamagui).
...
Note that the Tamagui babel plugin is optional for native.
...
Update your `babel.config.js` to include the optional `@tamagui/babel-plugin`, and the necessary `transform-inline-environment-variables` and `react-native-reanimated` (if using reanimated animations) plugins:
