...
Tamagui allows you to build your own set of components that are optimized with the compiler whenever they are used in your app.

<Notice>
  To skip to some real examples: The Nextjs-Expo-Solito template in `create-tamagui` is itself a [well structured monorepo](https://github.com/tamagui/tamagui/tree/master/starters); More complete, the [tamagui](https://github.com/tamagui/tamagui/tree/master/packages/tamagui) package itself.
</Notice>
...
<Notice>
  This guide assumes that you have knowledge on how to setup a monorepo using your
  preferred package manager. Keep in mind that this pattern isn't necessary to start.
</Notice>
...
Check the [configuration docs](/docs/core/configuration) for more details on this step.

You'll be creating a `tamagui.config.ts` at the the root of design system package. It will contain a full suite of tokens, themes, and fonts exported onto a single named `config` export.

### Step 3: Define and export components

Now, create and export the components. You can re-export components from `tamagui` or `@tamagui/core` as well. Let's create a Circle component
...
Now in your app, add `@ourapp/components` and `tamagui` (since we are extending it) to your package.json, and update your tamagui build configuration. Below are configs for Webpack projects, Next.js projects and React Native projects respectively, use the config that suits your project.

<CodeBlockWithTabs>
  <div id="webpack.config.js">
  ```js margin=false
  {
