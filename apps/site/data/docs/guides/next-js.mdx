...
This simply sets `fullySpecified` to false, becuase currently there is a conflict between Metro and Webpack that doesn't allow us to support fully specifying every import we use (specifically, Metro doesn't support `.native.ts` or `.web.ts` path extensions alongside specifying the import path).
