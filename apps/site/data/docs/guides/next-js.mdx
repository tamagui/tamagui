...
<InlineTabs id="router" defaultValue="pages">
<InlineTabs.List>
  <InlineTabs.Tab value="pages">Pages router</InlineTabs.Tab>
  {  /* <InlineTabs.Tab value="app">App router</InlineTabs.Tab> */}
</InlineTabs.List>

<InlineTabs.Content value="pages">
### next.config.js
...
Use `outputCSS` for better performance by generating CSS styles in build-time.
...
const tamaguiPlugin = withTamagui({
  ...
  // set to false if you never call addTheme or updateTheme TODO!!!
})
```
...
If you are using `outputCSS` option in [next.config.js](/docs/guides/next-js?#nextconfigjs), you should add `exclude` option to `getCSS()` in `_document.tsx`:
