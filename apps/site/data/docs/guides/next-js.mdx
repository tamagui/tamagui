...
module.exports = function (name, { defaultConfig }) {
  ...
  const tamaguiPlugin = withTamagui({
    ...
    components: ['tamagui'],
    // build-time generate you CSS styles for better performance
...
Add the reset to get more consistent performance:

```tsx
import '@tamagui/core/reset.css'
```

And then add this if you are using the `outputCSS` option for build-time CSS generation:
