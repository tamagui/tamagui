...
Currently, we have three templates available:

- `next-expo-solito` Expo, Expo Router, Solito and Next.js (with both `/pages` and `/app` dir)
- `expo` A prebuilt Expo app with Expo Router and Tamagui theming setup.
...
yarn native # Expo local dev (only for `next-expo-solito`)
```
...
In case you have any local packages feel free to add them to `/packages`.
...
You can also install the native library inside of `packages/app` if you want to get auto-import for that package inside of the `app` folder. However, you need to be careful and install the _exact_ same version in both packages. If the versions mismatch at all, you'll potentially get terrible bugs. This is a classic monorepo issue. We use `lerna-update-wizard` to help with this (you don't need to use Lerna to use that lib).
