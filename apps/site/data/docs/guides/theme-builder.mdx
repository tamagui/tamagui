...
<IntroParagraph>
  The new @tamagui/theme-builder package makes creating entire suites of themes for
  Tamagui much easier, with some performance benefits as well.
</IntroParagraph>

The Tamagui theme system is powerful - but in the past, too hard to fully leverage. With version 1.37 Tamagui released two new things to help make authoring theme suites much easier:

- A new `createThemeBuilder` chainable API exported from `@tamagui/theme-builder`.
- A corresponding compile-time `themeBuilder` option to help pre-generate themes.
