...
And then to set up your compiler to automatically build and watch your themes, add the following to your compiler config. For example with Next.js:
...
Moving further, we can read [the source for the `@tamagui/themes` package](https://github.com/tamagui/tamagui/blob/master/packages/themes/src/themes-new.ts), which this site uses, makes for a much fuller example. It handles a variety of more complex things:
