...
This guide goes into the why and how of `createThemeBuilder`. If you want to [skip to the API click go here](#createThemeBuilder), and if you just want a [complete example to copy-paste, go here](#complete-example).
...
The first layer of building a theme starts with a palette. A palette is a gradient within a single color, going from background to foreground:
...
Masks are really simple in practice, but require all the above context to get why they are useful.

Masks do one tiny thing: they take an existing template, and return a new one. We could create a mask called "shift-right" that takes this template:
...
Is is useful? Not really, but it explains what they do. Lets make a useful mask. We can revisit the "subtle" theme we discussed before. We want to take a base template, and return one where the background is less strong _and the text is less strong too_. If we look at our palette from before:

<Blog.ThemeBuilder.ExamplePalette showIndices />

That means if our base mask has background 0 and color 11, then our `subtlerMask` should return background 1 and color 10.

There's only one problem, and that is this: how does our mask function "know" to move the background index +1, but the color index -1? Well, it could guess and say "if index > 50%, move left, else move right", but this would be brittle. If we have a background-type thing that's right at the border, it could get shifted the wrong direction.

So this is where we introduce one final concept with templates: negative indices.

First, let's add negative indices to our palette:

<Blog.ThemeBuilder.ExamplePalette showIndices showNegativeIndices />

Now, lets modify our template:
...
Finally, we can modify `createTheme` to handle negative indices:
...
And it should work - returning the color from the right end of the palette, and the background from the left.

Now we can revisit our mask function `subtlerMask` and it will "know" when a value is meant to be a foreground or a background, and therefore can always shift them properly:
...
And that's all masks really are - functions that take a template and return a template. They help us especially with sub-themes and component themes, where often you want to take an existing theme, like `dark`, and then return a `dark_Button` theme where the background is a bit stronger, and the color a bit softer. So long as we define our templates using positive values for backgrounds and negative values for foregrounds, masks have all the info they need to make transformations.

In `@tamagui/create-theme` we've included a variety of masks we found useful:
