...
And finally, we can define a new `createTheme` that handles negative indices:
...
Now we have everything we need. We could easily rewrite our mask function `subtlerMask` to "know" when a value is meant to be a foreground or a background, and therefore can always shift them properly:
