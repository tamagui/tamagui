...
If given a `Variable` - which comes from a parsed theme or token returned from `createTamagui`, `useTheme` or `getTokens` (read more [on the useTheme docs](/docs/theme/use-theme)).
...
function MyButton(props) {
  const activeProps = useProps(props, {
    ...
    // defaults to not resolving token/theme values so they stay the same as they are input
    resolveValues: 'value'
  })
}
```
...
function MyButton(props) {
  const style = useStyle(props, {
    disableExpandShorthands: true,
    // 'value' resolves to the actual theme/token value on all platforms
    // 'auto' (default) resolves to CSS variable on web, value on native
    // 'variable' resolves to CSS variable on all platforms
    // default is 'auto'
    resolveValues: 'value',
...
function MyButton(props) {
  ...
  // theme and media are returned as they are used internally already in the hook
}
```
