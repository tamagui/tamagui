...
<Tabs.List>
  <TooltipSimple label="With Tamagui's default styles">
    <Tabs.Tab value="styled">
      <Paragraph size="$2">Styled</Paragraph>
    </Tabs.Tab>
  </TooltipSimple>
  <TooltipSimple label="No default styles, easy to customize">
    <Tabs.Tab value="unstyled">
      <Paragraph size="$2">Unstyled</Paragraph>
    </Tabs.Tab>
  </TooltipSimple>
  <TooltipSimple label="No dependency on Tamagui's core">
    <Tabs.Tab value="headless" label="No styles and no dependency on Tamagui's styling">
      <Paragraph size="$2">Headless</Paragraph>
    </Tabs.Tab>
  </TooltipSimple>
</Tabs.List>
...
To use the headless switch, you want to import it from the `@tamagui/switch-headless` package. This package has no dependency on `@tamagui/core`.
...
Using the `createCheckbox` export, you can create a fully custom checkbox without using any of the default styles. This is similar to `unstyled`, but it doesn't assume the props `size` or `unstyled` exist, and it won't automatically apply the `active` theme.

You must pass `CheckboxContext` as the `context` option to your Fraeme and Thumb styled components.
...
Using the `useCheckbox` API, you can make your own Checkbox from scratch. You could even do it without Tamagui's styling system if you wanted to.
