...
[We'd like to support this feature eventually](https://github.com/tamagui/tamagui/issues/190), but until then we've exported a `useListItem` hook, alongside `ListItemFrame`, `ListItemText`, `ListItemTitle`, `ListItemSubTitle`, and `listItemStaticConfig` that make it simple to create your own ListItem.
