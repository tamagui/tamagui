...
The simple API is very easy to use but may be harder to customize.

The composable API gives you more control over customization, since you can customize each composite component with the price of verbosity.

By default, both APIs are enabled, but Tamagui might do double-rendering to figure out which one you're using. To prevent that, you should set `buttonApi` when calling `createTamagui`. Like so:

```ts
createTamagui({
  // ...the rest of your config
  buttonApi: "comopsable", // or "simple"
})
```

## Headless

Tamagui offers a headless version of this component with no styling. Simply import button from `@tamagui/button/headless`. The headless import only supports the comopsable API.
...
Buttons extend Stack views inheriting all the [Tamagui standard props](/docs/intro/props) and use [`themeable()`](/docs/core/theme#themeable).
...
<Notice>
  Generally, if you need advanced customization on your buttons. You should go with either
  [Headless](#headless) or [Composable](#api-reference-composable) buttons.
</Notice>
