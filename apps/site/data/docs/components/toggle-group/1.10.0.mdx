...
<CodeBlockWithTabs>
  ...
  <div id="bun">
  ```bash margin=false
  bun add @tamagui/toggle-group
  ```
  </div>

  <div id="pnpm">
  ```bash margin=false
  pnpm add @tamagui/toggle-group
  ```
  </div>
</CodeBlockWithTabs>
...
`ToggleGroup.Item` extend Stack views inheriting all the [Tamagui standard props](notion://www.notion.so/docs/styling/props), plus:
