...
<CodeBlockWithTabs>
  ...
  <div id="bun">
  ```bash margin=false
  bun add @tamagui/radio-group
  ```
  </div>

  <div id="pnpm">
  ```bash margin=false
  pnpm add @tamagui/radio-group
  ```
  </div>
</CodeBlockWithTabs>
...
RadioGroup extend Stack views inheriting all the [Tamagui standard props](/docs/styling/props), plus:
...
RadioGroup.Indicator appears only when the parent Item is checked. it extends [ThemeableStack](/docs/components/stacks#themeablestack), getting [Tamagui standard props](/docs/styling/props) adding:
