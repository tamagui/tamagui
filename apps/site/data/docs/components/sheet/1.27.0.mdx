...
For now Sheet is the only component that requires you to put the children (Overlay, Frame, Handle) directly inside Sheet itself. We are working towards building a better setup so that you can abstract these into their own components if you wish.
