...
<Notice>
  Tamagui still tries to apply the styles even if you don't wrap them in `Group.Item`, but
  that's for backward compatibility and will probably change after a major release.
</Notice>
