...
<CodeBlockWithTabs>
  ...
  <div id="bun">
  ```bash margin=false
  bun add @tamagui/switch
  ```
  </div>

  <div id="pnpm">
  ```bash margin=false
  pnpm add @tamagui/switch
  ```
  </div>
</CodeBlockWithTabs>
...
`Switch` extends Stack views inheriting all the [Tamagui standard props](/docs/styling/props), plus:
...
`Switch.Thumb` extends Stack views inheriting all the [Tamagui standard props](/docs/styling/props), plus:
