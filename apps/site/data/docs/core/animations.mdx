...
<CodeBlockWithTabs>
  ...
  <div id='bun'>
  ```bash margin=false
  bun add @tamagui/animations-css
  ```
  </div>

  <div id='pnpm'>
  ```bash margin=false
  pnpm add @tamagui/animations-css
  ```
  </div>
</CodeBlockWithTabs>
...
To install it add to your package.json. We use [Moti](https://moti.fyi) as the driver, which is a very light layer above reanimated:
...
<CodeBlockWithTabs>
  ...
  <div id='npm'>
  ```bash margin=false
  npm add @tamagui/animations-moti react-native-reanimated
  ```
  </div>

  <div id='bun'>
  ```bash margin=false
  bun add @tamagui/animations-moti react-native-reanimated
  ```
  </div>

  <div id='pnpm'>
  ```bash margin=false
  pnpm add @tamagui/animations-moti react-native-reanimated
  ```
  </div>
</CodeBlockWithTabs>
...
<Notice>
  **Save bundle size** by removing `Animated` and `AnimatedFlatList` from
  ...
  option](/docs/guides/next-js) config option.
</Notice>
...
The animation hooks are heavy, which initially meant we either had to choose great performance or animations. We've settled on a trade-off. We track if the `animation` prop is set, and if so, we enable the hook. If it _is ever set, even just once_, then the hooks will continue to run for the remainder of the component lifecycle. This means if you ever plan to animate a component you should keep `animation` always set on the component props. You can disable it like so:
