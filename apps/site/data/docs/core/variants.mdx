...
In a style library you want to be able to succinctly add on conditional values that expand into groups of styles. Variants do just that. Before explaining more on why and how they work, and example:
...
<Notice>
  Notice the `as const` on the variant definition obect. This is necessary to please
