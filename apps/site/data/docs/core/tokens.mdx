...
Tamagui has some helpers that make working with variables easier, which are documented in [Extras](/docs/core/exports), namely [`getVariable`](/docs/core/exports#getvariable) which will return the CSS variable on web, but raw value on native, and `getVariableValue` which always returns the raw value.
