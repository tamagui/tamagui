...
You can use `getVariable` to get `variable` on the web, but `val` on other platforms.

```tsx
import { Stack, getVariable, useTheme } from '@tamagui/core'
import { SomeExternalComponent } from 'some-external-component'

const App = ({ shouldRerender }) => {
  const theme = useTheme()

  return (
    <SomeExternalComponent
      style={{
        backgroundColor: shouldRerender
