...
[The source](https://github.com/tamagui/tamagui/blob/master/packages/config-base/src/tamagui.config.ts) and [type output](config-base/types/tamagui.config.d.ts) of that config is also useful as a reference for building your own custom config.
