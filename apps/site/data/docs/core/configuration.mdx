...
```tsx
// the v2 config imports the css driver on web and react-native animation driver on native
...
This config is same as the one this site uses.
...
- `theme`: Define your design [theme](/docs/theme/theming), which map to CSS properties.
...
function App() {
  const [loaded] = useFonts({
    Inter: require("@tamagui/font-inter/otf/Inter-Medium.otf"), InterBold: require("@tamagui/font-inter/otf/Inter-Bold.otf"),
  });
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: `(beta) On iOS, this enables a mode where Tamagui returns color values using DynamicColorIOS significantly speeding up dark/light re-renders. If you use theme inversing or manually change from light to dark without also changing the native color scheme, this will break. We're working on accounting for these last edge cases before removing the beta tag.`,
    },
  ]}
/>
