...
Note that you don't have to set up a design system with `createTamagui` at all, and
...
web and the built-in Animated on React Native, but you can import
`animationsReanimated` if you'd like to swap either one out.
...
or `@tamagui/animations-moti` for [Moti](https://moti.fyi) animations. You can swap them out
