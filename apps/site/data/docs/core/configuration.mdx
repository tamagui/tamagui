...
One final optimization you can make if you are doing these server-side CSS patterns is setting the environment variable `TAMAGUI_DOES_SSR_CSS` to `true` for the client bundle only. Most bundlers will then be able to shake out all the theme-related CSS generation code from `@tamagui/core`, saving ~2kb of minified JS. If you set `outputCSS` option with Next.js, this will be set automatically.
