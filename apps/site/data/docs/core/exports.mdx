...
function MyButton(props) {
  const activeProps = useProps(props, {
    ...
    // if 'value' resolves to the actual theme/token value on the web
    // if `true` will resolve to CSS variable on web and value on native
    // default false leaves prop values as they are
