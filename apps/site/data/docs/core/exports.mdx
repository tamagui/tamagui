...
Given the specific name of a token, will return the value as either a variable on the web, or raw value on native. So if you define a size token with key `small` and value of `14`:
...
Similar to getToken, but always returns the raw value rather than the variable name. Iff you define a size token with key `small` and value of `14`:
