...
The `styled` function accepts all types of components, but if you're passing in a native component be sure to add the following:
...
This requirement allows our core to be light and not depend on the huge react-native library, which is key to allowing tree shaking and small bundle sizes on the web.
