...
Now your StyledSVG will properly type the `fill` property to accept token and theme values.  It will then map any values it receives to the style prop with any token or theme values resolved. Note the `as const`, until TypeScript 5 is more fully supported this improves the generated types.
