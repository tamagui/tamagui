...
[The source for that config is also useful](https://github.com/tamagui/tamagui/blob/master/packages/config-base/src/tamagui.config.ts) as a reference for a more complete config file.
