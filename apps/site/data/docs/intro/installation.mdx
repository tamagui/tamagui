...
You'll need to add a media query driver for responsive styles on native:

```
yarn add @tamagui/react-native-media-driver
```

And then add it to your configuration like this:
...
While `@tamagui/core` doesn't require any extra installation, if you are using `tamagui`, you may need to add a couple dependencies to your app and rebuild it to use certain components. This is because `tamagui` bundles many components together into one cohesive package.

For example, the Toast package requires `burnt` be added directly to your apps package.json.

Tamagui loads these native packages lazily, so you don't need to do anything right away.
