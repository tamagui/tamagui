...
```tsx
// the v2 config imports the css driver on web and react-native animated driver on native
...
If targeting web-only, want to save some bundle size, and are ok going without the React Native specific props like gesture handlers and onLayout, you can just use `web`:
