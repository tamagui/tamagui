...
That said, once you start using libraries from within the React Native ecosystem, you likely need some extra setup. Many of them use the special "platform specific" file extensions, like having `index.js` alongside `index.web.js`, which they expect the `index.web.js` to be picked up by the web. They also often import `react-native` directly, so if you're targeting web you need to set an alias from `react-native` to `react-native-web`.

Tamagui itself doesn't need any of this, but since you're probably going to add a variety of packages, you eventually will want to probably use one of the Tamagui bundler plugins.
...
This is also documented in the Configuration docs on the next page, but worth noting twice.
