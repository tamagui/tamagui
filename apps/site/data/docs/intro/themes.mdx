...
- dynamic themes are only works on the client side and will be ignored on the server side.
- difference between `updateTheme` ad `replaceTheme` as its name implies, `replaceTheme` will replace the entire theme, while `updateTheme` will only update the values that are passed in.
