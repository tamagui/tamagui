...
Note that the `@tamagui/babel-plugin` is completely optional, and on native Tamagui doesn't optimize as much as on web, so leaving it out is actually recommended. If much later on you feel the need for a bit more speed, you can try adding it. But the `babel-plugin-transform-inline-environment-variables` is necessary so that the `TAMAGUI_TARGET` environment variable gets properly set.
