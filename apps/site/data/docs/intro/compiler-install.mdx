...
<PropsTable
  data={[
    ...
    {
      name: 'emitSingleCSSFile',
      require: false,
      type: 'boolean',
      default: 'false',
      description: `(Experimental) Combines all CSS into one file, only available for Webpack for now. Must add lightningcss version 1.1.x`
    }
  ]}
/>
...
Tamagui has _experimental_ support for loading any component, even if it occurs somewhere outside your configured components modules. This is called "dynamic loading", for now. You can enable it with the setting `enableDynamicEvaluation` as seen above in the props table.
