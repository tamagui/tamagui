...
For more background read the [Compiler About](/docs/intro/why-a-compiler) page,
for performance see [Benchmarks](/docs/intro/benchmarks).

It's important to note the compiler is optional, and it's much easier to get
started by building your app out before you commit to the setup cost.
...
`experimentalAdvanceNativeFlattening` set to true in the above config object and
...
<PropsTable
  data={[
    ...
    {
      name: "experimentalAdvanceNativeFlattening",
      required: false,
      type: "boolean",
      typeSimple: "enum",
      default: "false",
