...
You don't need to setup anything to start using Tamagui. Go ahead and [skip to configuration](/docs/core/configuration).

This article helps set up the compiler, which, while it does add few helpful debug properties to your DOM, is not necessary to get configured to start your app.

The compiler runs with [Webpack](#webpack), [Next.js](#nextjs), [Vite](#vite), [Metro](#metro) and [Expo](#expo). There are dedicated guides for [Next.js](/docs/guides/next-js), [Expo](/docs/guides/expo), and [Vite](/docs/guides/vite).
