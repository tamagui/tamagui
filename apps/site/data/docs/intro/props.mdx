...
Groups are a new feature in beta that lets you define a named group, and then style children based on that name.
...
They also allow for targeting the parent pseudo state:

```tsx
<Stack group="card">
  <Text $group-card-hovered={{ color: 'white' }} />
</Stack>
```

This means when the parent Stack is hovered, the Text will turn white. The allowed psuedo modifiers are `hovered` (web only), `pressed`, and `focused`.

If you need types for these props you can define it alongside your tamagui type override:
