...
This choice is as old as software: target more platforms with one codebase but sacrifice UX and performance, or, target each platform individually but nearly double the time to ship each feature. With React Native it's been no different.
...
It's not just light bundles that are the limiting factor. Properly extracting even logic-ful components down to CSS for all the following is often more important in terms of overall app performance and responsitivy:

- Responsive styles (media queries)
- Themes, colors, sizing (CSS variables)
- Interactive styles (pseudo styles)

Media queries are basically table-stakes for sharing code, but running them as hooks de-opts web-apps such that even simpler apps bring modern browsers to a halt on resize. Pseudo styles and theme styles are also in places very sensitive to re-renders (especially as interactive changes often animate).

### How the compiler works

The Tamagui compiler optimizes four things:
