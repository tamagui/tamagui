...
React Native's raison dêtre is allowing sharing concepts across platforms along with _the UI that makes sense_. It does this well, allowing for large parts of the non-interface logic and smaller parts of the interface itself to be unified, saving a lot of developer time.

But as of today, there's still a large amount of interface that we don't share, but could, with only (somewhat) straightforward optimizations for the web platform. This article goes into how the Tamagui compiler achieves just that.
