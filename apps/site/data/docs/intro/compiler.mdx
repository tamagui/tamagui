...
React Native's raison dêtre is allowing sharing concepts across platforms along with _the UI that makes sense_. It does this well, allowing for large parts of the non-interface logic and smaller parts of the interface itself to be unified, saving much developer time.

But today, there's still a large amount of interface that we don't share, but could, with only (somewhat) straightforward optimizations for the web platform. This article goes into how the Tamagui compiler achieves just that.
...
The "Feels Native" column is the one to pay attention to here. What's really happening here is something that I've been thinking of as **The Frontend Trilemma**:
...
This choice is as old as software: target more platforms with one codebase but sacrifice UX and performance, or, target each platform individually but nearly double the time to ship each feature.
...
For all the great things its done, there are a few things React Native Web needs to do that that would individually have a big effect on improving code sharing:
...
Plus, support for the last three points would also all be a big win for developer experience for plain old React Native.
...
Tamagui also supports SSR _with media queries_ out of the box. This is another win for code sharing because SSR is one of the biggest performance gains you make to any app. Running `npm create tamagui-app` sets you up for this from the beginning with our [Next.js adapter](/docs/guides/next-js).
