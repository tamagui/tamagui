...
- A compiler that let you write comfy natural styles, even with logic, that optimize to each platform - ideally outputting just `div` and CSS on the web, or View on native.
