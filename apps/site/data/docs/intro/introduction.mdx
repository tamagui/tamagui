...
Tamagui is a complete suite of libraries for styling on React and React Native. It unifies the API completely between the two platforms, going further than react-native-web by providing a typed design system, animations, media queries, and pseudo styling that Just Works.

#### Core

A complete base for design systems, a styled() factory, and set of hooks that work the same across Native and Web, at ~17Kb gzipped.

It features **deterministic, easy to reason about styling** that by outputting to atomic CSS and always merging your styles based on the order it recieves them. This is a key feature of Tamagui, and something that doesn't exist outside of Facebook's internal StyleX library. It does this by expanding all styles as it finds them, and then merging them in-order, even across variants or when passed inline style props.
