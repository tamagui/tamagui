...
<IntroParagraph theme="alt1">
  Tamagui is a complete suite of libraries for styling on React and React Native.
</IntroParagraph>
...
- Apply styles conditionalls inline or with powerful typed variants.
...
- Expands CSS shorthands at each step to enure merging is always correct.
- Pseudos always win, and Media queries always win in a predictable manner.

#### Universal

- Works on Native and Web the exact same across the entire API surface area.
- No dependency core means you can leave React Native out entirely for web-only use cases.
