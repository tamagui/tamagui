...
Now only when the parent Stack is hovered the Text will turn white. The allowed
...
Now the Text will be white, but only when the Stack matches the media query
