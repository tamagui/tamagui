...
<IntroParagraph large>
  React Native is a great platform for building fluid, native feeling apps but when you
  share UI with the web you [either accept a slow app, or write it
  twice](/docs/intro/compiler).
</IntroParagraph>

<IntroParagraph>
  Tamagui solves this with a novel optimizing compiler that turns JS into CSS, evaluates
  away inline styles, and flattens component trees - greatly increasing render
  performance.
</IntroParagraph>

It turns natural typed inline styles, even with logic, into CSS that beats bests even hand-written styling, thanks to tree shaking and view flattening. Tamagui supports the full set of React Native APIs in a 0-dependency core library.

<Spacer />
<BenchmarkChartWeb />
<Spacer />

On the web, psuedo and responsive styles are over 10x faster, while Lighthouse scores go up anywhere from 10-20% just by toggling the compiler on.
...
#### How Tamagui transforms code

Tamagui can evaluate a wide variety of code to flatten your component trees down:

<Spacer />
<TamaguiExamplesCode />

The key to Tamagui's performance gains lie in **view flattening**: the compiler analyzes code, even across module boundaries, and evaluates even logical code via AST transformation and node VM-powered code evaluation.

It will fully flatten your custom styled components like `<Circle />` into plain old `<div />` (or `<View />`, on native) and it does so often thanks to its smart evaluator.
