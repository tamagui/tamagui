...
description: Ship cross-platform React apps in ½ time with 2x performance.
by: nate
# relatedIds:
#   - 'tamagui-alpha'
---
...
<IntroParagraph large>
  React Native is a great platform for building fluid, native feeling apps. But sharing
  that code to the web means [either accepting slow apps, or writing them
...
It can make inline and modern typed style syntax into more efficient code than hand-tuned CSS, or any other style library due to flattening.

<Spacer />
<BenchmarkChartWeb />
<Spacer />

We've built a lightweight [core library for styling](/docs), [static is the optimizing compiler](), and [tamagui is a full-featured UI kit built on top]().
...
> The tamagui.dev home page scores 13% higher lighthouse scores with the compiler on.

We've built `tamagui` on top of the core style library, a complete suite of components and hooks designed to adapt to feel native on every platform, yet still be completely customizable thanks to a uniquely powerful [theme system](/docs/intro/themes), sizing props, and Radix-inspired composable component API surfaces.
...
- Shorthands for quick authoring
- Merges styles down the three without worry! Typed, just destructure props.
...
[Static](#whats-is-static) is the optimizing compiler. It supports web and native through a plugin interface that has adapters for Webpack, Vite, Next, React Native, and Expo. Over a dozen major improvements to the compiler have made their way in since beta. Some of the highlights are:
