...
<IntroParagraph large>
  React Native is a great platform for building mobile apps, but have a choice when it
  comes to sharing UI: either [accept a slow app, or rewrite it](/docs/intro/compiler).
</IntroParagraph>
...
It turns natural typed inline styles, even with logic, into CSS that beats bests even hand-written styling thanks to CSS extraction and view flattening. Tamagui supports the full set of React Native APIs in a 0-dependency core library.

<Spacer />
<BenchmarkChartWeb />
<Spacer />

On the web, render performance is faster, but psuedo and responsive styles really shine with over 10x the speed, and Lighthouse scores gain anywhere from 10-25%.
