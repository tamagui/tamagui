...
<IntroParagraph>
  Tamagui fixes these three downsides with a React style library that works with a novel
  optimizing compiler. The compiler output often bests hand-crafted CSS while still
  letting you ship fast with typed inline styles and a powerful style interface.
</IntroParagraph>
...
[**Tamagui Core**](#overview-of-tamaguicore) (`@tamagui/core`) is a style library for React Native and web that supports 100% of the React Native API surface in a 0-dependency library. It adds all the [goodies of modern style libraries](#overview) into a cohesive package. [Skip to what's new](#whatsnew-in-tamaguicore).

[**Tamagui Static**](#overview-of-tamaguistatic) (`@tamagui/static`) is an optimizing compiler on top of Core, turning even inline styles sprinkled with logic into atomic CSS and minimal JS, flattening your view tree for maximum performance. [Skip to what's new](#whatsnew-in-tamaguistatic).

[**Tamagui UI**](#overview-of-tamagui) (`tamagui`) is a component kit built on top of the incredibly powerful theme primitives of `core`. That, alongside composable-component APIs and complete size control enables a new level of control not seen before in a UI kit. [Skip to what's new](#whatsnew-in-tamagui).

The next section gives a high level code overview of how Tamagui works.
...
We've seen the addition of seven new components. Each comes with composable component API for much more complete control over each piece of the components visuals and behavior.
...
Like the existing components in Tamagui, every new component is sizable, themable, and able automatically styled using your design system.
...
The monorepo now includes `apps/kitchen-sink`, a native app that demos every component that comes in Tamagui. This has helped us quickly iterate and improve components and their correctness across Android and iOS. Android especially received a wide variety of fixes during the betas.
...
The `next-expo-solito` starter repo has had extensive polish as well, simplifying the setup, removing the need for watch commands, and properly building to production across all features for web, iOS and Android.
...
More than any other section in this release, the biggest amount of effort for 1.0 went into correctness. Tamagui has steadily landed fixes across every feature and component since Beta, and the difference between early this year and now is astounding, with a whole ton of edge cases handled automatically now.

### Community

We've been building a great community [especially on Discord](https://discord.gg/4qh6tdcVDa). Check out [the Community page](/community) for more information including starter kits and Figma files.
...
We've expanded our testing signficantly, automated releases, added stricter linting and fuller integration tests across more platforms. The `release.ts` script is much imrpoved and dependencies checked for a variety of errors in publishing.

---

## Going forward

Tamagui is stable and free of all blocking bugs across every feature, but there's a ton to go to make every part of it simpler, easier, faster, type-correct and generally correct.

Writing about what comes after 1.0 is a blog post to itself, and we're planning on a public Roadmap. For now, [next.md](https://github.com/tamagui/tamagui/blob/master/next.md) is a living document of upcoming features and fixes.

I'd like to make this project work financially long-term, so there are plans for a few very interesting add-ons that provide powerful functionality when working on a team, and dealing with advanced theme customization.

One way I'm experimenting with that is granting early access to new components, features, and the upcoming Studio and Docs apps at a discounted rate.
...
This release once again uses inspiration and OSS code from a variety of other libraries and developers. We've had 38 new contributors, which is amazing, and some significant correctness improvements have landed from outside contributions in the last month.

A special thanks to [Vercel](https://vercel.com), [Codingscape](https://codingscape.com), [QuestPortal](https://www.questportal.com) [BeatGig](https://beatgig.com) and all the individuals who helped with incredibly significant early sponsorships ❤️.

A huge thanks to [Radix](https://www.radix-ui.com/), [Floating UI](https://floating-ui.com/), [Moti](https://moti.fyi), and [Framer Motion](https://github.com/framer/motion) which are used or inspired parts of Tamagui.
