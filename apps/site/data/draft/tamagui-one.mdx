...
<IntroParagraph>
  Tamagui is universal style system for React taken to the next level thanks to an optimizing
  compiler. It outputs beautifully platform-targeted code to either React Native or Web, allowing
  you to [ship faster, share more code across platforms, and net better
  performance](/docs/intro/compiler) all at once.
</IntroParagraph>
...
- 100% of features have SSR and React Server Components support.
- Comprehensive typed design system generation and use.
- Prop-order based styling means no more CSS specificity fights!
- Beautiful inline styles that work with the full API surface area.
- Typed flat style props + shorthands give a Tailwind-like authoring experience while being able to spread overridden styles consistently.
