...
Testing now covers far more of `core` and `static`, as well as a complete integration test that verifies the entire flow of starting a Tamagui app with `npm create tamagui-app` all the way to Playwright ensuring the final output is correct and functional.
...
- Windows support for `npm create tamagui-app` can break, requires WSL
...
- Some versions of Next.js dont properly load CSS on subsequent pages
