...
description: Cross-platform React apps in ½ time with 2x performance.
by: nate
...
<IntroParagraph large>
  React Native is a great platform for building mobile apps, but when you target the web
  as well, you have to either [accept a slow app, or forego sharing
  code](/docs/intro/compiler).
</IntroParagraph>

<IntroParagraph>
  Tamagui uses a novel optimizing compiler that enables more code sharing and incredible
  performance. It turns JS into CSS, evaluates away inline styles, generates
  platform-targeted code, and flattens component trees, resulting in simpler code and a
  faster runtime.
</IntroParagraph>

Tamagui core supports every React Native API in a 0-dependency library that smooths over the gap between web and native with helpful web-inspired styling support. It turns natural typed inline styles, even with logic, into CSS that bests even hand-written code thanks to tree-shaking and component flattening. When it optimizes, your whole app gets a lot faster:

<Spacer />
<BenchmarkChartWeb />
<Spacer />

On the web, render performance is much improved especially with interactive and responsive styles net 10x or more speed thanks targeting The Platform™️ and avoiding all JS. It improves Lighthouse scores 10-25%.
