...
/**
 * Provides middleware to Node.js Express-like servers. Used by the Tamagui
 * Vite dev server plugin as well as production Node.js implementation.
 */
export function tamaguiMiddleware({
  dev,
  cache,
  indexTemplate,
  getServerEntrypoint,
  devServer,
}: TamaguiMiddlewareArgs) {
  if (dev && devServer) {
   ...
   */
  const webPolyfills =
    !globalThis.fetch || !globalThis.ReadableStream ? import('../utils/webPolyfill.js') : undefined
    ...
    try {
      ...
      if (typeof handleRequest !== 'function') {
        if (entrypointError) {
          ...
          throw new Error(
            'Something is wrong in your project. Make sure to add "export default renderTamagui(...)" in your server entry file.'
          )
        }
      }
