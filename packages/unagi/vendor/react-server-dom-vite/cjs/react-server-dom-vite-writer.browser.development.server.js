  ...
  ;(function () {
    ...
    function closeWithError(destination, error) {
      if (typeof destination.error === 'function') {
        // @ts-ignore : This is an Error object or the destination accepts other types.
    ...
    function switchContext(newSnapshot) {
      ...
      if (prev !== next) {
        if (prev === null) {
          // @ts-ignore : This has to be non-null since it's not equal to prev.
          pushAllNext(next)
        } else if (next === null) {
          popAllPrevious(prev)
    ...
    function resolveModelToJSON(request, parent, key, value) {
      ...
      } // @ts-ignore : bigint isn't added to Flow yet.
    ...
    function renderToReadableStream(model, options) {
      ...
      var stream = new ReadableStream(
        ...
        }, // @ts-ignore  size() methods are not allowed on byte streams.
        {
          highWaterMark: 0,
        }
      )
