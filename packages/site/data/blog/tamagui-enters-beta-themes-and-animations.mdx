...
The two big new pieces in the beta are **Animations**, and **Themes**.

---

## Overhauled Themes 🎨

Themes have been completely redone, and they're really fun. They now default to names that map to standard style names with pseudo postfixes, so for `color`, the alternates `colorHover`, `colorFocus`, and `colorPress` are all definable.

Building off that, theme's are now truly nestable - so you can have `dark` and `light`, and below that `dark_blue` and `light_blue`, and then different tints below that (like `dark_blue_alt1`).
