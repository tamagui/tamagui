...
Basically, if you have a base `dark` theme and a `light` theme, you may want to have "tinted" versions of those themes. For example `pink-light` and `pink-dark`. That way, instead of having to hard-code colors like `$pink3` all over your app, you can use generic values like `$background3` and instead wrap that component tree in `<Theme name="pink" />`.
