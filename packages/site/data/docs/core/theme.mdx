...
Basically, if you have a base `dark` theme and a `light` theme, you may want to have "tinted" versions of those themes. For example `pink_light` and `pink_dark`. That way, instead of having hard-coded color values like `$pink3`, you can use generic theme values like `$background` and instead wrap that component tree in `<Theme name="pink" />`.
