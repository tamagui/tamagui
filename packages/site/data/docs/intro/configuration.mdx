...
- You must be define all `size` keys, but the rest can be partial.
...
The rest of the tokens are flat. Note that `space` and `size` generally also share keys, though space can have negative keys as well.
