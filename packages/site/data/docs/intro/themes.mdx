...
Because themes are powerful and can adapt to every component, we're working on better high level libraries to generate themes from colors. Until then, you can roll your own, or use/fork our `@tamagui/theme-base` package.
...
Tamagui has settled on matching keys mostly to style properties, which pseudos defined like `backgroundHover`. This makes it easy for themes to apply to any component.
