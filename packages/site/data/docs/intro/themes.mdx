...
const Button = styled(Stack, {
  // note: this name will match it to and theme ending with `_Button`:
  name: 'Button'
})
```
...
And now you can have any theme customize these two components by defining `_Button` and `_ButtonText`.
...
type ThemeName = keyof allThemes
