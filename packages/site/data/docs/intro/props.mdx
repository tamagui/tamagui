    ...
    },
    {
      ...
      description: (
        <span>
          ...
          filter out any nullish child elements, and the insert a spacing component between the
          remaining of the size specified by the value.
        </span>
      ),
    },
    {
...
Inline styles have a double speed benefit over StyleSheet.create: they require fewer imports and fewer lines of code, and they save you from jumping between the top and bottom of your file whenever you want to change a style so you can see exactly whats happening where it's happening.
...
Changing between a style that's determined by a ternary or conditional and one that's static shouldn't be a "hard" choice. With StyleSheet there's some friction as you have to either inline it or extract it to the bottom. With inline styles (and the compiler) you no longer have any friction: convert any prop to have a ternary and it works the same, and extracts the same.
