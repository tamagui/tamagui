...
Tamagui lets you **share way more code between web and native apps - all while improving DX, performance, and accessibility**. It turns all styles, even inline styles, responsive queries and themes, into efficient extracted atomic CSS (or on native, hoisted into a top-level StyleSheet.create).

This is a **win-win-win**: it's more performant, easier to write and maintain, and renders on native and web the same. Typically you'd trade performance for DX, or both for cross-platform compatibility. With Tamagui, you get all three at once.

The compiler even evaluates away many common logical operations, conditionals, and spreads. It flattens any fully analyzable views into a much faster simple HTML tags on web, or View/Text on native.

<Image title="Tamagui logo" src="/blog/introducing-tamagui/hero.png" height={200} />
