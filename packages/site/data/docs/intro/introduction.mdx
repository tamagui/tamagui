...
- **Responsive styles** — media queries that avoid JS when possible
- **Themes** — that also avoid running JS
- **Inline styles** — that extract at compile-time out of render

Today, you can't really share code between Native and Web without resorting to doing media queries, themes and styles generally in JavaScript. But on the web, this is especially slow.

Many of these components could be easily shared if not for this limitation. So Tamagui works backwards from ideal syntax - both an inline style syntax as well as hooks - and then on the web generates [CSS media queries](https://developer.mozilla.org/en-US/docs/Web/CSS/Media_Queries/Using_media_queries) and [CSS variables](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties) when possible, which are designed for this purpose and are very fast.

The Tamagui compiler goes further and flattens what would have been a `View` or `Text` into a plain `div` or `span`, flattening your tree.
