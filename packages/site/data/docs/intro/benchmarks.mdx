...
Note, the next two tests are not really testing styling performance at all, as they're mostly testing the internals of the component. We think they don't represent anything close to a typical application, but we've left them in to be charitable.

In general, the plan is to add a new test that does represent better a good mix of use cases as the current tests here are all very micro.
...
Tamagui will also dramatically outperform for media queries and themes, which will be in upcoming tests.
