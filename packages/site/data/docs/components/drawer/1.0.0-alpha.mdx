---
title: Drawer
description: A simple drawer component
name: text
---

# Drawer

<Description>Show content above in a bottom sliding drawer.</Description>

<HeroContainer>
  <DrawerDemo />
</HeroContainer>

```jsx hero template=Drawer

```

<Highlights features={[`Animation and dragging on native and web.`]} />

## Usage

Drawer extends the excellent (@gorhom/bottom-sheet)[https://github.com/gorhom/react-native-bottom-sheet]. Note this will bring along [react-native-reanimated](https://github.com/software-mansion/react-native-reanimated) and will require some extra configuration with webpack:

```tsx
oneOf: [
  {
    test: /(bottom-sheet).*\.[tj]sx?$/,
    use: [
      {
        loader: 'babel-loader',
        options: {
          plugins: [
            'react-native-reanimated/plugin',
            '@babel/plugin-transform-react-jsx',
          ],
        },
      },
    ],
  },
  // tamagui loader
]}
```

### Drawer props

All the props from [react-native-bottom-sheet](https://gorhom.github.io/react-native-bottom-sheet/props), plus:

<PropsTable
  data={[
    {
      name: 'open',
      required: false,
      type: 'boolean',
      description: `Show drawer.`,
    },
    {
      name: 'onChange',
      required: false,
      type: '(showing: boolean) => void',
      description: `Called on change show (if dismissed).`,
    },
    {
      name: 'showHandle',
      required: false,
      type: 'boolean',
      description: `Show a handle at top.`,
    },
    {
      name: 'hideBackground',
      required: false,
      type: 'boolean',
      description: `Don't show a background container element.`,
    },
    {
      name: 'children',
      required: false,
      type: 'JSX.Element',
      description: `What's inside the drawer.`,
    },
  ]}
/>
