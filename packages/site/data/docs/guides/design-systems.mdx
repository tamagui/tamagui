...
- We're using `@tamagui/esdx` to build this package, which is a small script built around `esbuild` and `rollup-plugin-flat-dts` that makes sure you output your components _with JSX preserved_ into `./_jsx`.
