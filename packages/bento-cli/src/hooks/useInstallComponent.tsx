...
export const installComponent = async ({ component, setInstall, install }) => {
  ...
  if (hasPackagesAndUIDir()) {
      ...
      components.map((component) => {
        ...
        if (componentSchema?.moveFilesToFolder) {
          ...
          if (!toFolder) {
            ...
            if (!existsSync(uiSubPathDir)) {
              mkdirSync(uiSubPathDir, { recursive: true })
            }
            fs.writeFile(
              path.join(
                process.cwd(),
    ...
    components.map(
      (component) =>
        // createDir({ component, uiPath: ['src', 'components', 'ui'] })
