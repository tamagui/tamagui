...
const DeepChild = memo(({ depth, id, onLayoutFired }: { depth: number; id: string; onLayoutFired: () => void }) => {
...
export function OnLayoutStressCase() {
  ...
  const gridItems = useMemo(() =>
    Array.from({ length: 40 }, (_, i) => (
      <GridItem key={i} id={i} sizeMultiplier={gridSize} onLayoutFired={onLayoutFired} />
    )),
  ...
  return (
    <YStack flex={1} padding="$2">
      ...
      <XStack gap="$2" flexWrap="wrap" padding="$2" backgroundColor="$background">
        <Button testID="btn-resize-width" size="$2" onPress={() => setWidthMultiplier((v) => v === 1 ? 1.5 : 1)}>
          toggle width ({widthMultiplier}x)
        </Button>
        <Button testID="btn-toggle-expand" size="$2" onPress={() => setExpanded((v) => !v)}>
          toggle expand ({expanded ? 'on' : 'off'})
        </Button>
        <Button testID="btn-resize-grid" size="$2" onPress={() => setGridSize((v) => v === 1 ? 1.3 : 1)}>
          toggle grid ({gridSize}x)
        </Button>
        <Button testID="btn-resize-container" size="$2" onPress={() => setContainerWidth((v) => v === 600 ? 400 : 600)}>
      ...
      <XStack gap="$3" padding="$2" backgroundColor="$backgroundHover" flexWrap="wrap">
        <Text testID="stat-total" fontSize={11}>total: {stats.totalCallbacks}</Text>
        <Text testID="stat-avg" fontSize={11}>
          avg: {stats.totalCallbacks > 0 ? Math.round(stats.totalTime / stats.totalCallbacks * 100) / 100 : 0}ms
        </Text>
        <Text testID="stat-max" fontSize={11}>max: {Math.round(stats.maxTime)}ms</Text>
        <Text testID="stat-last-batch" fontSize={11}>lastBatch: {stats.lastBatchSize}</Text>
        <Text testID="stat-last-time" fontSize={11}>lastTime: {stats.lastBatchTime}ms</Text>
      </XStack>
      ...
      <ScrollView flex={1}>
        <YStack testID="stress-container" width={containerWidth} gap="$3">
          ...
          <YStack testID="section-deep">
            <Text fontSize={12} fontWeight="bold">Deep Nesting (10x6 depth)</Text>
          ...
          <YStack testID="section-grid">
            <Text fontSize={12} fontWeight="bold">Grid (40 items)</Text>
          ...
          <YStack testID="section-list">
            <Text fontSize={12} fontWeight="bold">List (20 items)</Text>
            <YStack>
              {listItems}
            </YStack>
          </YStack>
          ...
          <YStack testID="section-siblings">
            <Text fontSize={12} fontWeight="bold">Sibling Groups (5x10)</Text>
            <YStack gap="$2">
              {siblingGroups}
            </YStack>
          </YStack>
