...
describe('SheetScrollableDrag - RNGH Integration', () => {
  ...
  it('should show RNGH enabled', async () => {
    if (isAndroid()) return
    await expect(element(by.id('sheet-scrollable-drag-rngh-status'))).toHaveText('RNGH: ✓ enabled')
  })
  ...
  it('Case 1: swipe DOWN at scrollY=0 should drag sheet, NOT scroll', async () => {
    ...
    await device.takeScreenshot('case1-before')
    ...
    await device.takeScreenshot('case1-after')
    ...
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 1 result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text)
  ...
  it('Case 3: drag sheet up from position 1 should NOT scroll simultaneously', async () => {
    ...
    const scrollAfterDragDown = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 3 - scroll after drag down:', (scrollAfterDragDown as any).text)

    await device.takeScreenshot('case3-at-position1')

    // now drag UP back to position 0
    await element(by.id('sheet-scrollable-drag-scrollview')).swipe('up', 'fast', 0.6)
    await new Promise(r => setTimeout(r, 800)) // extra wait for scroll reset

    await device.takeScreenshot('case3-after-drag-up')

    // EXPECTED: sheet back at position 0, scroll should still be 0 (no simultaneous scroll)
    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 3 result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text)
  ...
  it('Case 4: scroll down, then swipe down should scroll back to 0 first', async () => {
    ...
    const scrollBefore = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 4 - scroll before swipe down:', (scrollBefore as any).text)

    await device.takeScreenshot('case4-scrolled-down')

    // now swipe DOWN - should scroll back first
    await element(by.id('sheet-scrollable-drag-scrollview')).swipe('down', 'slow', 0.6)
    await new Promise(r => setTimeout(r, 500))

    await device.takeScreenshot('case4-after-swipe-down')

    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 4 result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text)
  ...
  it('Case 5: HANDOFF - scroll to 0 then drag sheet in one gesture', async () => {
    ...
    await device.takeScreenshot('case5-scrolled')
    ...
    await device.takeScreenshot('case5-after-handoff')

    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Case 5 HANDOFF result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text)
  ...
  it('Case 6: multiple direction changes without getting stuck', async () => {
    ...
    await element(by.id('sheet-scrollable-drag-scrollview')).swipe('up', 'slow', 0.3)
    await new Promise(r => setTimeout(r, 400))
    const scrollAfter = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    const scrollVal = parseInt((scrollAfter as any).text.replace('ScrollView Y: ', ''), 10)
    console.log('Direction change test: scroll Y after swipe up:', scrollVal)
    if (scrollVal < 30) {
      throw new Error(`Expected scroll > 30 after swipe up at position 0, got ${scrollVal}`)
    }
    ...
    const finalPos = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const finalScroll = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    console.log('Direction change test: final state - Position:', (finalPos as any).text, 'Scroll:', (finalScroll as any).text)
  ...
  it('Case 7: drag UP from position 1 - scroll locked during drag, final position 0', async () => {
    ...
    await device.takeScreenshot('case7-at-position1')
    ...
    await device.takeScreenshot('case7-after-drag-up')

    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    const maxScrollAttr = await element(by.id('sheet-scrollable-drag-max-scroll-y')).getAttributes()
    console.log('Case 7 result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text, 'MaxScroll:', (maxScrollAttr as any).text)
  ...
  it('Case 9: HANDOFF - drag UP from position 1, continue into scroll in one gesture', async () => {
    ...
    await device.takeScreenshot('case9-at-position1')
    ...
    await device.takeScreenshot('case9-after-long-swipe')

    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-scroll-y')).getAttributes()
    const maxScrollAttr = await element(by.id('sheet-scrollable-drag-max-scroll-y')).getAttributes()
    console.log('Case 9 HANDOFF UP result - Position:', (posAttr as any).text, 'Scroll:', (scrollAttr as any).text, 'MaxScroll:', (maxScrollAttr as any).text)
    ...
    if (maxScrollValue < 50) {
      throw new Error(`Expected max scroll > 50 after handoff, got ${maxScrollValue}. Handoff from sheet to scroll failed!`)
    }
  })
  ...
  it('Case 8: rubber band at top - dragging up keeps sheet at top without scrolling', async () => {
    ...
    await device.takeScreenshot('case8-at-top')
    ...
    await device.takeScreenshot('case8-after-resist')

    const posAttr = await element(by.id('sheet-scrollable-drag-position')).getAttributes()
    const scrollAttr = await element(by.id('sheet-scrollable-drag-max-scroll-y')).getAttributes()
    console.log('Case 8 result - Position:', (posAttr as any).text, 'MaxScroll:', (scrollAttr as any).text)
    ...
    await expect(element(by.id('sheet-scrollable-drag-max-scroll-y'))).toHaveText('Max scroll Y: 0')
  })
})
...
async function navigateToSheetScrollableDrag() {
  ...
  await waitFor(element(by.id('home-sheet-scroll-test')))
    .toBeVisible()
    .withTimeout(5000)
  await element(by.id('home-sheet-scroll-test')).tap()
