...
describe('ThemeLevels', () => {
  beforeAll(async () => {
    ...
    await waitFor(element(by.id('theme-levels-root')))
      .toExist()
      .withTimeout(180000)
  })
  ...
  it('composes relative levels in both schemes', async () => {
    ...
    await expect(element(by.id('light-button-name'))).toHaveText('light_level2_level2')
  ...
  it('keeps the red theme through the Button level', async () => {
    await expect(element(by.id('red-level-button-name'))).toHaveText('light_red_level2')
  })
})
