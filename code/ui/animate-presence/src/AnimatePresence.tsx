...
> = ({
  ...
  let forceRender = React.useContext(LayoutGroupContext).forceRender ?? useForceUpdate()
  ...
  const allChildren = React.useRef(new Map<ComponentKey, ReactElement<any>>()).current

  const exiting = React.useRef(new Set<ComponentKey>()).current
