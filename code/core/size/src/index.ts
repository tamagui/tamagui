  ...
  const resolve = ((name: SizeTableName<Table> = defaultSize) => {
    if (!(name in values)) {
      if (process.env.NODE_ENV !== 'production') {
        console.error(
          `Unknown size "${String(name)}" — this size table only has: ${Object.keys(values).join(', ')}. Size tables are user-owned named scales; the default skins take size tokens (like "$4") instead. Falling back to "${String(defaultSize)}".`
        )
      }
