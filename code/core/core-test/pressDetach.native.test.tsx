...
// mirrors a selectable row: press visuals only while unselected. the select
  ...
  return (
    <TamaguiProvider config={conf} defaultTheme="light">
      <View
        ...
        pressStyle={selected ? undefined : { backgroundColor: 'red' }}
...
describe('press detach', () => {
  test('conditional pressStyle removed mid-press does not latch the press wash', async () => {
    vi.useFakeTimers()
    ...
    // re-attaching the press visuals must not resurrect the wash
    await tap(rendered!, 'reset')
    expect(backgroundOf(rendered!, 'row')).toBe('white')
  })
})
