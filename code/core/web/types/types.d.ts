 ...
 * arms").
 */
type FlatClauseBait<T, PrefixOnly extends boolean> = `${ClauseModifierName}:` | `${ClauseModifierName}:${ClauseModifierName}:` | (PrefixOnly extends true ? never : `${ClauseModifierName}:${FiniteString<T>}` | `${FiniteString<T>} ${ClauseModifierName}:`);
export type FlatStyleValue<T, PrefixOnlyBait extends boolean = false> = T | FlatClauseBait<T, PrefixOnlyBait> | (string & {});
export type WithThemeValues<T extends object> = {
    [K in keyof T]: (ThemeValueGet<K> extends never ? K extends keyof ExtraBaseProps ? T[K] : FlatStyleValue<T[K] | 'unset'> : FlatStyleValue<GetThemeValueForKey<K> | Exclude<T[K], string> | 'unset', K extends ColorKeys ? true : false>) | (K extends SafeAreaValueKeys ? 'safe' : never);
};
