...
export const COMPATIBILITY: readonly CompatibilityRow[] = [
  ...
  {
    area: 'style',
    subject: 'inline reset placement',
    keys: [
      // the twelve tags RSD gives its inline reset on web
      'web.a.textAlign',
    ...
    rsd: 'resets inline elements per tag with textDecoration: none and textAlign: inherit',
    tamagui:
      'resets text-decoration once in the :where() host reset and leaves textAlign alone',
    reason:
      'the host reset covers every inline tag at zero specificity, and textAlign already inherits, so declaring either per tag changes nothing',
  },
  {
    area: 'style',
    subject: 'element defaults RSD leaves to the browser on web',
    keys: [
      // del, ins, s and u keep only their semantic decoration; the margin and
      // padding reset RSD never declared for br, del, ins, kbd, optgroup,
      // option, s and u moved to the :where() host reset
  ...
  {
    area: 'style',
    subject: 'browser-stylesheet undo lives in the :where() host reset',
    keys: [
      // margin and padding RSD declares per tag; the tables no longer repeat
      // them because every host carries is_DOM and the generated
      // `:where(.is_DOM)` rule resets them at zero specificity
      'web.a.margin',
      'web.a.padding',
      'web.article.margin',
      'web.article.padding',
      ...
      'web.bdo.margin',
      'web.bdo.padding',
      'web.blockquote.margin',
      'web.blockquote.padding',
      'web.button.margin',
      'web.button.padding',
      'web.code.margin',
      'web.code.padding',
      ...
      'web.sup.padding',
      'web.textarea.margin',
      'web.textarea.padding',
      'web.ul.margin',
      'web.ul.padding',
      // border-style RSD declares on the four inline-block controls; the
      // generated per-display `:where(<tag>.is_DOM)` rule resets it the same way
      'web.button.borderStyle',
      'web.input.borderStyle',
      'web.select.borderStyle',
      'web.textarea.borderStyle',
    ],
    rsd: 'declares the margin, padding and control border-style undo per tag',
    tamagui:
      'declares them once in the `:where(.is_DOM)` host reset generated from the tag display table',
    reason:
      'atomic per-tag defaults lose to page CSS while a zero-specificity host rule needs no repeating; the conformance comparison sees only the semantic table defaults, and the generated reset test pins the rules to the display table',
  },
  {
    area: 'style',
  ...
  },
  {
    ...
    keys: ['web.img.aspectRatio', 'web.img.objectFit'],
    ...
    tamagui:
      'sets objectFit, leaves the inline reset to the :where() host reset like any other inline element, and lets the compiler turn the width and height props into an aspect ratio on native',
    reason:
