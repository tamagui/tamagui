...
import { TAGS, TAG_NAMES, TAG_WEB_DEFAULTS } from '../src/tables/tags'
...
/**
 * The element defaults for one tag, as Tamagui style props: the semantic
 * defaults plus the web-only ones. Undoing the browser default stylesheet
 * (the old margin/padding/text-decoration/border-style reset) is not here; it
 * lives in the zero-specificity `:where(.is_DOM)` rules in `htmlReset.ts`
 * so page CSS still reaches html.* hosts.
 */
...
/**
 * The zero-specificity reset for html.* hosts, as CSS text for the design
 * system. Every host carries `is_DOM`; the selector lists come from the
 * `display` column of the tag table so the table stays the only source, while
 * the declarations are the fixed browser-stylesheet undo. No rule sets
 * `display`: that is what leaves block vs inline to the browser.
 */
export function generateHtmlReset(): string {
  return format(htmlResetSource(), 'htmlReset.ts')
}

function htmlResetSource(): string {
  const hosts = (display: 'inline' | 'inline-block') =>
    TAG_NAMES.filter((tag) => TAGS[tag].display === display)
      .map((tag) => `${tag}.is_DOM`)
      .join(', ')
  const rules = [
    ':where(.is_DOM) { margin: 0; padding: 0; }',
    `:where(${hosts('inline')}) { text-decoration: none; }`,
    `:where(${hosts('inline-block')}) { border-style: solid; }`,
  ]
  return `${[
    ...
    ' */',
    `export const htmlResetCSS = \`${rules.join('\n')}\``,
    '',
  ].join('\n')}`
}
...
function htmlSource(): string {
  const components = TAG_NAMES.map((tag) => {
    ...
    const base = isText ? 'htmlTextStaticConfig' : 'htmlStaticConfig'
    const element = isText ? 'TamaguiTextElement' : 'TamaguiElement'
    const props = isText ? 'HtmlTextProps' : 'HtmlViewProps'
    ...
    return [
      ...
      // only the semantic defaults: the browser-stylesheet undo is the
      // :where(.is_DOM) reset, not atomic default props
      ...(defaults ? [`  defaultProps: { ${defaults} },`] : []),
      `})`,
    ].join('\n')
  })
...
if (import.meta.main) {
  ...
  writeFileSync(join(dom, 'htmlReset.ts'), generateHtmlReset())
  writeFileSync(join(dom, 'standaloneHtml.ts'), generateStandaloneHtml())
  console.info(
    `generated domEventProps.native.ts, html.tsx, html.native.tsx, htmlReset.ts and standaloneHtml.ts for ${TAG_NAMES.length} tags`
  )
}
