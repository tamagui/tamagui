...
[**Tamagui Static**](#overview-of-tamaguistatic) (`@tamagui/compiler`) is an optimizing compiler on top of Core, turning even inline styles sprinkled with logic into atomic CSS and minimal JS, flattening your view tree for maximum performance.
...
Lets get familiar with both `@tamagui/core` (the styling library) and `@tamagui/compiler` (the compiler), and how the latter optimizes the former.
...
## What's new in `@tamagui/compiler`
