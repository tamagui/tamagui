...
client-side. Its SSR support is advanced without jumps or flickers, even when
...
This replaces the previous `toggledStyle` prop on ToggleGroup with the more
consistent `activeStyle` naming. Icons inside these components also
automatically size based on `SizableContext`.
