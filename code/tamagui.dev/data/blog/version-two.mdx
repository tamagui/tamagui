...
<IntroParagraph large>
  ...
  It's optimizing compiler turns logical, typed, inline styles into optimally
...
If you want to skip to the good stuff: [what's new in version two](#new-in-v2).
...
- Improved theme generation packages, and the
...
- **Removed deprecated v2 config exports.** The `/v2` exports from `@tamagui/config`,
  `@tamagui/themes`, and `@tamagui/shorthands` have been removed. These were the
  original config format from early Tamagui versions - not to be confused with
  Tamagui v2 itself. The current config is now at v5. If you were using
  `@tamagui/config/v2`, migrate to `@tamagui/config/v5` or the default export.
...
- **React Native 0.82+ and React 19+ required.** Tamagui 2 targets React Native
  ...
  significant new style features and simplified internals.
...
- Native portals no longer auto-import React internals hack. If you want native
  ...
  `@tamagui/portal/setup-native` at app startup. See [Portal docs](/docs/components/portal).
...
state updates. Further, Motion leverages the
[Web Animations API (WAAPI)](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API),
which runs animations off the main thread. Both together make for much smoother
web animations with springs.
...
This replaces the incosistent methods of active styling found across components
in v1.
...
your tree. This is a significant performance win - in testing with large apps,
we found it greatly improved UX by eliminating the overhead of mounting full
overlay components inline.
...
Scoping also solves React context issues with portals - since the content lives
at the root inside your providers, all context is available without needing to
re-propagate it. For Sheet in particular, using scoping avoids needing a portal
entirely on native.
...
experimental theme optimization setting has been removed - theme optimization
now works automatically and much more effectively. This means better performance
on native without any configuration needed.
...
The problem: on native, the default default portal implementation doesn't
...
aren't available inside portaled content. The re-propagation also adds overhead.
...
This uses React Native's native `createPortal` API (Fabric or Paper) to preserve
the React fiber tree, so all context flows through automatically. No more
manually re-propagating theme context or navigation state into your sheets and
dialogs.
...
Agents generate it's code exceptionally well becuase it's been in wide use and
...
**[Add Even](https://addeven.com)**. We built a production app for a company late
last year in record time — three weeks from start to approval in app stores —
