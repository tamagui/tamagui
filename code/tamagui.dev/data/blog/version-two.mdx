...
The `@tamagui/core` bundle has been reduced by ~40% (from ~37KB to ~25KB
gzipped). This was achieved through several optimizations:

- **Removed color parsing on web** - Colors like `red` or `#fff` are now passed
  directly to CSS instead of being parsed and converted to rgba. Web browsers
  handle color parsing natively.
- **Removed responder events** - The
  `@tamagui/react-native-use-responder-events` package has been removed
  entirely. React Native has built-in responder support.
- **Tree-shakeable CSS generation** - When using the Vite/webpack plugin with
  `outputCSS` enabled, all runtime CSS generation code is automatically
  eliminated from your bundle.

If you're using the compiler to extract CSS at build time, you get the full
...
[Toast](/docs/components/toast) has been completely rewritten with a simple,
Sonner-inspired API. Gone are the complex provider/controller patterns - now you
just call `toast()`:
...
For colored toasts, wrap with Theme - no internal type logic needed:
...
The old `Toast`, `ToastProvider`, `ToastViewport`, `useToastController`, and
`useToastState` APIs have been removed.
