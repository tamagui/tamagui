...
- Native portals no longer auto-import React internals hack. If you want native
  portals for better context preservation, add `import '@tamagui/native/setup-portal'`
  at app startup. See [Portal docs](/docs/components/portal).
...
`react-native-gesture-handler` for smooth scroll-to-drag gesture coordination.
Install `react-native-gesture-handler` and add the setup import before any
Tamagui imports:
...
See the [Sheet docs](/docs/components/sheet) for full details.
