...
- Removed `<Theme inverse />` and the `themeInverse` prop on all components
- Removed the `maxDarkLightNesting` and `cssStyleSeparator` settings

#### Theme Inverse Removal

The `inverse` prop on `<Theme>` and the `themeInverse` prop on components have
been removed. Supporting inverse themes properly with CSS and SSR required
complex, bloated code.

If you need inverse theme behavior, you can achieve this by creating an
"inverse" sub-theme and using `<Theme name="inverse">` instead:
...
const themes = {
  light: { ... },
  dark: { ... },
  light_inverse: { ...darkTheme },
  dark_inverse: { ...lightTheme },
}
...
You can also use the [Theme Builder](/docs/guides/theme-builder#inverse-themes)
to generate this.
