...
**Style compatibility** - V5 defaults to `styleCompat: 'web'`, so flex shorthand
behavior and raw numeric `lineHeight` match CSS on web. Set
`styleCompat: 'react-native'` for Yoga flex behavior and absolute numeric
`lineHeight`, or use `lineHeight="24px"` when you want an absolute line height in
web mode. Font line-height tokens stay absolute.
...
- **`textShadow`** - CSS text-shadow shorthand with token support:
  ...
  supported.
- **`border`** - CSS border shorthand with token support:
  `border="1px solid $borderColor"`. On native, expands to individual width,
  style, and color properties.
- **`outline`** - CSS outline shorthand with token support:
  `outline="2px solid $outlineColor"`. On native, expands to `outlineWidth`,
  `outlineStyle`, and `outlineColor`.
...
Pseudo-style props (`hoverStyle`, `pressStyle`, `focusStyle`) now support their
own `transition` property. This enables CSS-like semantics where entering a
state uses that state's transition, and exiting uses the base transition:

```tsx
<Square
  transition="1000ms"  // slow exit (back to base)
  hoverStyle={{
    transition: '200ms',  // fast enter
    backgroundColor: 'red',
  }}
/>
```

This pattern creates snappy, responsive interactions where elements react
quickly to user input but settle back smoothly. Works with group pseudo states
too (`$group-card-hover`). See the
[animation docs](/docs/core/animations#pseudo-style-transitions) for details.
...
- **`tamagui generate-css`** - Generate the `tamagui.generated.css` file from your config.
...
- **Config v5 changes from v4.** Media query names change (`2xl` → `xxl`, `2xs`
  → `xxs`), `styleCompat` defaults to web semantics, and raw numeric
  `lineHeight` is unitless on web. See
  [Config v5 docs](/docs/core/config-v5).
- **Absolute numeric line heights should move to strings.** In web mode,
  `lineHeight={24}` means a CSS multiplier of `24`. Use `lineHeight="24px"` for
  an absolute 24px line height, or keep font line-height tokens like
  `lineHeight="$4"`.
- **React Native 0.81+, React 19+, and TypeScript 5+ required.** Tamagui 2
  targets React Native 0.81, React 19, and TypeScript 5 and above with New
  Architecture support. This enables significant new style features, improved
  type inference, and simplified internals.
- Default flex and position styles align with browser defaults, while
  `styleCompat: 'react-native'` is available for Yoga flex behavior.
