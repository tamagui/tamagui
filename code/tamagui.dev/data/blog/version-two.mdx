...
- **React Native 0.83+ required** for native apps. This enables full support for
  web-standard accessibility props.
- Removed React Native-specific accessibility props (`accessibilityLabel`,
  `accessibilityRole`, etc). Use web-standard `aria-*` and `role` props instead.
- Removed `focusable` prop. Use `tabIndex` instead.
...
(React Native 0.76+), they resolve to raw values:
