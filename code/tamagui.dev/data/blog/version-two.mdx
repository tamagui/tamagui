...
- **`backgroundImage`** - CSS `linear-gradient` and `radial-gradient` syntax with token support.
...
Pointer events (`onPointerDown`, `onPointerUp`, `onPointerMove`, `onPointerEnter`,
`onPointerLeave`, `onPointerCancel`) now work cross-platform. On native, they're
mapped to touch events with a normalized event shape matching the web API:

```tsx
<View
  onPointerDown={(e) => {
    // works on web and native
    console.log(e.clientX, e.clientY, e.pointerId)
    e.target.setPointerCapture(e.pointerId)
  }}
  onPointerMove={(e) => {
    // receives events outside bounds when captured
  }}
  onPointerUp={(e) => {
...
The `setPointerCapture` and `releasePointerCapture` APIs are supported for drag
scenarios where you need move events to continue firing even after the pointer
...
- **Shadow colors** output opacities using `color-mix()` which may break snapshot
  tests or any code testing the expected parsed format.
