...
- Tabs `activationMode` now defaults to `'manual'` instead of `'automatic'`. This
  means focusing a tab no longer changes the active tab - you must click or press
  Enter/Space. This matches web standards.
...
drivers (like using CSS for simple transitions and Reanimated for complex
springs), and it enables the compiler to better optimize your code by knowing
statically which driver a component uses.
