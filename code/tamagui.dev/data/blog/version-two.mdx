...
- **Removed deprecated v2 config exports.** The `/v2` exports from
  ...
  default export.
- **Config v5 changes from v4.** Animations are now imported separately
  (`@tamagui/config/v5-css` etc.), media query names changed (`2xl` → `xxl`,
  `2xs` → `xxs`), and style defaults align with React Native. Migration is
  straightforward - see [Config v5 docs](/docs/core/config-v5).
...
**Progress** - Now SSR-friendly with percentage-based transforms on web that
render correctly without client-side hydration. Bouncy animations no longer
overshoot past the container edges.
