...
The solution: import the setup module once at app startup, before any Tamagui imports:
...
[Sheet](/docs/components/sheet) now supports optional integration with
`react-native-gesture-handler` for native-quality gesture coordination between
the sheet and scrollable content. This provides:

- **Smooth scroll-to-drag handoffs** - Seamlessly transition between scrolling
  content and dragging the sheet without lifting your finger
- **No gesture conflicts** - Sheet and ScrollView gestures coordinate properly
- **Native-quality feel** - Matches the behavior of system sheets like Apple
  Maps or Google Maps

To enable, install `react-native-gesture-handler` and add the setup import
before any Tamagui imports:
...
Then use `Sheet.ScrollView` for scrollable content inside sheets:
...
<Sheet>
  ...
  <Sheet.Frame>
    <Sheet.ScrollView>
      {/* Scrollable content with smooth gesture handoff */}
    </Sheet.ScrollView>
  </Sheet.Frame>
</Sheet>
```

Without the gesture handler setup, Sheet falls back to React Native's built-in
PanResponder which works well for basic use cases. See the
[Sheet docs](/docs/components/sheet) for more details.
