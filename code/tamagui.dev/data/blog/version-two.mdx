...
expensive gesture handling, and automaticall trimming optional theme generation
code.
...
- **`backgroundImage`** - Web-style gradient, `linear` and `radial`.
- **`boxShadow`** - Full CSS box-shadow support including multiple shadows,
  spread, and inset. Accepts strings with your tokens.
- **`backgroundImage`** - CSS `linear-gradient` syntax with token support.
...
wholheartedly recommend it for your web apps.

We spent a lot making it special with the **new Tamagui 0 re-render path**.
Tamagui now supports an `avoidReRenders` option for animation drivers. Style
changes don't even pass through React.
...
memoization for better runtime performance. The compiler analyzes components and
automatically inserts `useMemo` and `useCallback` where beneficial, reducing
unnecessary re-renders without any code changes on your part.

To use Tamagui with the React Compiler in your own app with React 18, add the
runtime polyfill:
...
React 19 includes this runtime natively, so no additional dependencies are
needed.
...
- \*\*Shadow colors output opacities using color-mix() which may break snapshot
  tests or any code testing the expected the parsed format.
