...
- **`textShadow`** - CSS text-shadow shorthand with token support:
  `textShadow="2px 2px 4px $shadowColor"`. On native, only single shadows supported.
...
as an [animation driver](/docs/core/animations-motion), and we can
wholeheartedly recommend it for your web apps.

We put a lot of effort into making it special with the **new Tamagui 0 re-render
path**. Tamagui now supports an `avoidReRenders` option for animation drivers.
Style changes don't even pass through React.

Combined with Motion's **off-thread animation support** via the
[Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API),
and we've noticed huge gains in UX on apps.
