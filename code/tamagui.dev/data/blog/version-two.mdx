...
- **Able to share 100% between web and native.** Many other solutions focus on
  one platform more than others, or implement their features in sub-par ways on
  web.
- **Great performance and platform feeling on both web and native.** As a small
...
- **Fast, type-safe, properly mergeable inline styling.** Inline styles are a
  leap forward, but the lack of type safety or easy ability to merge and group
  your styles is their biggest drawback. Tamagui is fully type safe, lets you
  control how you merge styles, and optimizes it all nearly perfectly even in
  the face of complicated logic.
- **A first-class, composable components based UI kit.** Our UI kit doesn't just
...
experiences on all platforms with fully shared code. We're proud to be used by
companies like [Uniswap](https://uniswap.org) who used Tamagui to unify their
featureset across platforms and greatly simplify their codebase.

### Improvements during v1

Tamagui improved continuously during version one:

- Hundreds of new tests, stricter lints, dependency checks and a more robust
  release process.
- Created
...
**Native CI:** Two dedicated workflows handle native testing - one for Detox E2E
tests on both iOS and Android, and another for Maestro flow tests covering
