...
```tsx
<Variables
  values={{ backgroundHover: 'blue4' }}
  themes={{ dark: { backgroundHover: 'blue2' } }}
>
  <SettingsPanel />
</Variables>
```
...
The `themes` map scopes values per theme name — `dark` and `light` compile
under scheme selectors so color-scheme flips are render-free, and any other
theme name (`blue`, `red_alt1`) compiles under that theme's class. On native,
the patch rides the existing granular theme system, and JavaScript readers
like `useTheme()` see the same values on both platforms.
