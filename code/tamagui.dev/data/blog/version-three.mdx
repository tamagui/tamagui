...
description: React Strict DOM and Tailwind syntax on one compiler, plus simpler APIs, a web-first core, and stronger composition.
by: nate
---

<IntroParagraph large>
  Tamagui 3 is about simplification, performance, and alignment with the web:
  fewer legacy aliases, fewer hidden wrappers, more web-standard APIs, and a
  cleaner path between Tamagui props, composable UI, and Tailwind utility syntax.
</IntroParagraph>
...
The headline of v3 is two features that arrive together: React Strict DOM and a
package-selected Tailwind frontend.

React Strict DOM means `html` is the authoring surface. You write real semantic
elements, and they render as actual DOM on web and their React Native
equivalents on native. The Tailwind frontend puts utility classes in `className`
...
Read the full [Tamagui Tailwind post](/blog/tailwind-mode) for compiler details
...
Version 3 unifies this around statically-shaped style fragments across three arities:
...
Within each tier, later definitions win. Resolver chains execute in parent-first order, so a child component's resolver overrides its parent's resolver for conflicting properties. Setting a property to `undefined` in a resolver means absent, allowing lower-tier styles to show through.

### Compiler path

This unified model simplifies static extraction:
- **Lane A (available today):** The compiler evaluates `styled.dynamic` callbacks and `.resolve` chains inside its evaluation sandbox for callsites with statically-known props.
- **Lane B (v4 zero-runtime path):** Conforming bodies with static keys and no spreads will extract to static CSS classes with CSS variables for dynamic values.
...
also gone. V2 could spread one style across its base prop, pseudo-style objects,
