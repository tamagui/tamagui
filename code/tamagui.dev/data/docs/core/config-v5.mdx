...
- **`styleCompat: 'web'`** - `flex` follows CSS shorthand semantics and raw
  numeric `lineHeight` on web is unitless. v4 used `'legacy'` with
  `flexBasis: auto`.
- **React Native compatibility mode** - Set `styleCompat: 'react-native'` if you
  want `flex` to match Yoga and raw numeric `lineHeight` to stay absolute.
...
V5 defaults to `styleCompat: 'web'`, which is the recommended v2 setting.
  ...
  data={[
    {
      name: 'web',
      description:
        'Default. flex follows CSS shorthand behavior, and raw numeric lineHeight on web is emitted as a unitless CSS number.',
    },
    {
      name: 'react-native',
      description:
        'Matches Yoga for flex expansion and keeps raw numeric lineHeight as an absolute React Native value.',
    },
  ]}
/>

A `legacy` mode preserves the v4 flex behavior (`flexBasis: auto`). It's deprecated and only intended as a short-term escape hatch during v4 to v5 upgrades.

In `web` mode, `lineHeight={1.2}` means `line-height: 1.2`, so the browser
multiplies it by the element's font size. To keep an absolute line height, use a
length string:
...
Font tokens are unchanged. `lineHeight="$4"` still resolves to the absolute
font token value on both web and native.

V2 is a large step toward web alignment, but because the Tamagui ecosystem
grew up around React Native flex semantics, the base `View` keeps
`flex-shrink: 0` until v3. It's the only style default not aligned to CSS in
`web` mode. The `flex` shorthand still emits `flex-shrink: 1`, so
`<View flex={1} />` shrinks as CSS expects; only plain `<View />` keeps the
RN-style default. See
[styleCompat in configuration](/docs/core/configuration#stylecompat) for the
full reference.
...
<PropsTable
  data={[
    ...
    },
    {
      name: 'styleCompat',
      description:
        '"web" - Uses CSS flex and unitless lineHeight semantics. Use "react-native" for Yoga flex and absolute numeric lineHeight, or "legacy" for v4 flexBasis: auto behavior.',
    },
  ]}
/>
...
2. **Update media query names** - `2xl` → `xxl`, `2xs` → `xxs`, `max2Xl` →
   `max-xxl`
3. **Review `styleCompat`** - v5 defaults to `web`; use `react-native` for Yoga
   flex and absolute numeric line heights, or `legacy` for v4 flex behavior
4. **Review raw numeric `lineHeight`** - use `"24px"` strings for absolute
   lengths in web mode
5. **Review position** - v5 doesn't set `defaultPosition` (defaults to `static`)
6. **Review theme colors** - v5 uses Radix Colors v3 with slightly different
   values
