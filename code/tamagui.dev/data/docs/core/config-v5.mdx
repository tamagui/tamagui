...
As a temporary measure while you're upgrading from v4 and want to preserve the
old visual behavior, use `defaultProps` in your config:
...
We want to move away from component themes in general as they just are a bit too
tricky to understand. Likewise, we'd like to move away from defaultProps
eventually as well. In the long run what we recommend is just setting colors or
the `theme` props directly on your components. And if you need to share them,
then creating your own abstracted component you re-use.
