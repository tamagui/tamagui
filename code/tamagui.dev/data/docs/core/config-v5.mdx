...
<Notice>
  On native mobile, `touchable` is always true and `hoverable` is always false. Native
  macOS and Windows targets invert those defaults because they use hover-capable pointers.
</Notice>
