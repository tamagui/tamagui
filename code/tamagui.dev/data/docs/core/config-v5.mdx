...
```tsx fileName="tamagui.config.ts"
import { createV5Theme, opacify } from '@tamagui/themes/v5-builder'
...
<View bg="accent-background" color="accent-color" />
```
...
To restore longhand props while extending `defaultConfig`, override
`onlyAllowShorthands`:
...
`onlyShorthandStyleProps` is a separate, narrower setting for border, outline, and
shadow style props. Setting it to `false` does not override `onlyAllowShorthands`.
