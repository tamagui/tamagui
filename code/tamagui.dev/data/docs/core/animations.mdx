...
For **web-only** applications, we recommend a hybrid approach:

- **[CSS](/docs/core/animations-css)** - Use for load-time sensitive pages like landing pages and marketing sites. By avoiding the JavaScript bundle of Motion/Reanimated on static pages, you can significantly improve your Lighthouse scores.
- **[Motion](/docs/core/animations-motion)** - Use for dynamic, playful animations in your app. Motion provides buttery smooth spring animations that run off-thread via WAAPI, making it ideal for interactive components like sheets, popovers, and page transitions.

For **cross-platform** apps:

- **[Reanimated](/docs/core/animations-reanimated)** - Our recommendation for native. It provides off-thread performance with advanced spring physics.
- **[Moti](/docs/core/animations-reanimated)** (Reanimated driver) - If you're using Reanimated on native, use the Moti driver on web too. This avoids shipping both Reanimated and Motion, which would add significant bundle size.
- **[Motion](/docs/core/animations-motion)** - Only use on web if you're NOT using Reanimated elsewhere in your app. If you need Reanimated for native, stick with Moti on web to avoid duplicate animation libraries.

The best part? You can switch between CSS, Motion, Reanimated, and Moti using the `Configuration` component (see [Combining Drivers](#combining-drivers) below).
...
You can use multiple animation drivers in the same app using the `Configuration` component. This is perfect for optimizing performance - use lightweight CSS animations for your marketing pages and outer layout, then swap to Motion for richer animations once users are in your app.

Here's the pattern we use in production:

### 1. Create Two Animation Configs

```tsx
// animationsRoot.ts - CSS for outer layout
import { createAnimations } from '@tamagui/animations-css'

export const animationsRoot = createAnimations({
  quick: 'ease-out 100ms',
  medium: 'ease-out 300ms',
  bouncy: 'cubic-bezier(0.175, 0.885, 0.32, 1.275) 350ms',
})
```

```tsx
// animationsApp.ts - Motion for in-app experience
import { createAnimations } from '@tamagui/animations-motion'
...
### 2. Use CSS as Default in Your Config
...
export const config = createTamagui({
  animations: animationsRoot, // CSS animations as default
  // ...
})
```

### 3. Swap to Motion with Configuration

```tsx
// app/(app)/_layout.tsx - Your authenticated app layout
import { Configuration } from 'tamagui'
import { animationsApp } from './animationsApp'

export function AppLayout({ children }) {
  return (
    <Configuration animationDriver={animationsApp}>
      {children}
    </Configuration>
  )
}
...
Now your marketing pages load with lightweight CSS animations, while your logged-in app experience gets the full power of Motion's spring physics.

<Notice theme="green">
  The `Configuration` component only affects its children, so you can have different animation drivers for different parts of your app tree.
</Notice>
...
variant of the animated child before it runs its exit animation. This is useful
