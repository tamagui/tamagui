...
<IntroParagraph>
  The root `html.*` export gives one component tree semantic HTML on web and the
  corresponding React Native primitives on native.
</IntroParagraph>

Each member is an ordinary Tamagui component, so it accepts Tamagui style props
alongside the DOM props for that element.
