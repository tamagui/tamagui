...
You can also import `html` from `@tamagui/style`.

`html.*` is the recommended default for new code. [`View` and `Text`](/docs/core/view-and-text) remain fully supported, and
every `html.*` member is built on one of them, so they take the same style
props, tokens, themes, and variants.
...
`tamagui/dom` and `@tamagui/style/dom` entries are deprecated implementation
