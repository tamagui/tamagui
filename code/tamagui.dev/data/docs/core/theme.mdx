...
Then, you just give your component a `theme="pink"` prop, or wrap it in `<Theme name="pink" />`. Note that you can leave out the `light_` or even `light_pink` prefix, Tamagui will nest properly so long as the parent is set.
