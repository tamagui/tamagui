...
Before `FontLanguage`, you could get custom fonts with some effort. Simply create a `body` font, and a `body_french` font, and switch it out like so on relevant text components: `<html.span fontFamily={isFrench ? 'body_french' : 'body'} />`.
...
<FontLanguage body={isFrench ? 'french' : 'default'}>
  <html.span fontFamily="body">Hello world</html.span>
</FontLanguage>
```
...
```tsx
import { FontLanguage, html } from 'tamagui' // or '@tamagui/style'
