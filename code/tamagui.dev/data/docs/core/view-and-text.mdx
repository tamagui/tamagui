...
For new code, reach for the [`html.*`
primitives](/docs/core/html-primitives) first: they render a semantic element
on web and the matching native primitive on native. View and Text stay fully
supported, and every `html.*` member is built on one of them.
