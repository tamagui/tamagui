---
title: View & Text
description: View and Text with the full Tamagui style API
---

<IntroParagraph>
  Tamagui's `View` and `Text` match the React Native primitives and accept the
...
`span`. For semantic tags, set [`render`](/docs/core/styled#render), or see the
[HTML elements](/docs/core/html-primitives), which are in early development.
...
Inline styles and [shorthands](/docs/core/shorthands) compile away, so prefer them for one-off styles:
...
<Notice theme="blue">
  Style prop order matters. [Read more here](/docs/intro/styles#order-is-important)
</Notice>
...
<html.div
  ...
  onPointerEnter={() => console.log('hovered')}
  opacity="press:0.8"
...
Use these events alongside `press:`, `hover:`, and `focus:` clauses to create interactive components. See the [Style API](/docs/intro/styles) for interaction styling.
...
<PropsTable
  data={[
...

    {
      ...
      description: (
        <span>
          A CSS transition string (<code>"200ms ease-out"</code>), a preset name from your
          createTamagui configuration (<code>"bouncy"</code>), or an object for per-property
          control (<code>{`{ preset: 'bouncy', opacity: '100ms' }`}</code>). See the{' '}
          <a href="/docs/core/animations">Animations documentation</a>.
        </span>
      ),
    },

    {
      ...
      description: (
        <span>
          Apply a theme or sub-theme, such as <code>theme="inverse"</code>, to
          this component and the components below it.
        </span>
      ),
    },
...

    {
      ...
      description: (
        <span>
          Adds a className. In Tailwind mode, supported utility classes compile to
          Tamagui styles on web and native. Unknown classes pass through on web.
        </span>
      ),
    },
...

    {
      ...
      description: (
        <span>
          Used for controlling keyboard and assistive device focus. This replaces the old <code>focusable</code> prop.
        </span>
      ),
    },
