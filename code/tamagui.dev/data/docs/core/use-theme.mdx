...
change. When the [`fastSchemeChange` setting on createTamagui](/docs/core/configuration#settings)
...
This is a more advanced use for building custom hooks or components. Pass an
explicit theme name to read the matching sub-theme below the current scheme:
...
The above `useTheme` hook returns the `dark_green` theme. V3 does not append a
component name or inspect uppercase theme segments.
