...
For the full list of accepted style properties, see the
[Styles page](/docs/intro/styles).

Start from an `html.*` member for new components. It renders the semantic
element on web and the matching native primitive on native, and `styled()`
adds base styles and typed variants while preserving Tamagui's compiler
optimizations:

```tsx
import { GetProps, html, styled } from '@tamagui/core'

export const RoundedSquare = styled(html.div, {
  borderRadius: 20,
})
...
`styled()` wraps any Tamagui component. `View` and `Text` stay fully
supported for code that matches the React Native primitives.
