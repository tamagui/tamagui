...
### Combining accept with variants

`accept` becomes especially powerful when combined with spread variants. For example, you can create an SVG icon component that accepts `color` and `size` variants using your design tokens, and have the resolved values forwarded as SVG-specific props like `fill`, `width`, and `height`:
...
Here the `...color` spread variant maps any color token (like `$red10` or a theme value like `$color`) to the `fill` prop. The `accept` config then tells Tamagui that `fill` should be resolved as a color token, and `width`/`height` as size tokens. This means the underlying component receives fully resolved values:

```tsx
<StyledSVG color="$blue10" size="$4" />
```

You can also mix exact matches alongside spread variants in the same variant group:
...
When `color="red"` is passed, the exact variant is used. For any other color token, the spread variant resolves it.

### accept with style objects
