...
And if you'd like to add new props on top of the existing props, you can pass them in for the first generic type argument of `createStyledHOC`:
...
const Custom = createStyledHOC(View)<ExtraProps>((props) => {
  // ...
  return null
})
```
