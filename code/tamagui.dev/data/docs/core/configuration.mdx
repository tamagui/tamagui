...
<Notice theme="blue">
  We recommend starting with [`@tamagui/config/v6`](/docs/core/config-v6), our preset
...
configuration, go to [the Config v6 page](/docs/core/config-v6).
...
const systemFont = createFont({
  ...
  lineHeight: {
    // numeric config values are absolute pixels; 1 will be 22
    2: 22,
  },
  weight: {
    1: '300',
...
const config = createTamagui({
  ...
  // Shorthands
  // Adds <html.div m={10} /> to <html.div margin={10} />
...
Font configuration and component styles intentionally treat numbers differently.
A numeric font `lineHeight` remains an absolute pixel value for compatibility
with existing configs. A numeric string is a ratio, while a px string is an
explicit absolute length:
...
Used as font tokens, `body` and `fixed` resolve to 24 pixels, while `relaxed`
resolves to 1.5 times the effective font size. In Tamagui component styles,
`lineHeight={1.5}` is also a ratio. Use `lineHeight="24px"` when the authored
style itself needs an absolute value. Tamagui does not infer either meaning from
...
```tsx
const App = () => (
  <html.span fontSize="lg" lineHeight="lg" fontFamily="body" color="white">
    Hello world
  </html.span>
)
```
...
Custom variables sit alongside your themes: they merge into every base theme,
resolve like theme keys in style props and `useTheme()`, and can be
[redefined for any subtree](/docs/core/theme#theme-updates) with `<ThemeUpdate>`.
...
Values can be literals, references to theme keys or tokens (`'border-color'`,
`'size.4'`), or per-scheme `{ light, dark }` pairs.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      default: false,
      description: `Set settings.selectionStyles to generate ::selection styles for text selection on the web.`,
    },
    {
    ...
    },
    {
      ...
      type: `'string' | 'object'`,
      default: `undefined`,
      description: `Restrict conditional style values to one syntax. Flat values accept both the string form (bg="red hover:blue") and the object form (bg={{ default: 'red', hover: 'blue' }}); set this to keep one of them, which narrows the types, makes the runtime warn on the other in development, and tells tamagui generate-prompt and the skills to only show that form.`,
    },
    {
    ...
    },
    {
      ...
      description: `Chooses the render performance tradeoff. "updates" tracks which theme and media keys each component reads so changes re-render only their consumers, at a small per-component first-render cost. "first-render" skips per-key tracking for the fastest initial render; theme and media changes still apply but re-render coarsely. This is a startup-level setting, don't change it at runtime.`,
    },
  ]}
/>
...
<Notice>
  ...
  apply to Metro/Expo or static sites. The static v6 defaults are already generated, so
  this is mainly relevant when chasing the last few Lighthouse points.
</Notice>
