...
<IntroParagraph>
  Configuration connects your tokens, themes, media queries, animations,
  shorthands, and settings to every Tamagui component.
</IntroParagraph>

Start with [`@tamagui/config/v6`](/docs/core/config-v6) for default tokens,
...
>>>>>>> origin/feat/v3-lineheight-multiplier
    2: 22,
  },
  weight: {
    1: '300',
...
Sizing holds the control size ladder: named rungs of token keys plus the rules
that derive pixel geometry per rung. V6 configs wire in `defaultSizing`, so most
apps only touch this key to add a size:
...
const config = createTamagui({
  ...
  sizing: {
    ...defaultSizing,
    sizes: { ...defaultSizing.sizes, xxl: { ...defaultSizing.sizes.xl } },
  },
})
```
