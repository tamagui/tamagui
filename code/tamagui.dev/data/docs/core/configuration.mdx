...
We made `@tamagui/config` to make it easy to generate a robust Tamagui configuration.

It comes with a recommended default configuration and themes, as well as an easy way to make your own themes with `createThemeSuite`.

```tsx fileName="tamagui.config.ts"
import { config, createThemeSuite } from '@tamagui/config/v4'
