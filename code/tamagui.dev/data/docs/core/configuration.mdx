...
```tsx fileName="tamagui.config.ts"
import { createFont, createTamagui, createTokens, isWeb, prefixTokens } from 'tamagui'
...
export const tokens = createTokens({
  ...
  ...prefixTokens('space', { ...size, '-1': -5, '-2': -10 }),
  'radius-0': 0,
  'radius-1': 3,
  'zIndex-0': 0,
  'zIndex-1': 100,
...
You access tokens by their name. The style property you use decides which
category is searched, so `color="white"` finds `color-white` and
`borderRadius="sm"` finds `radius-sm`. Write the full name to reach across
categories.
