...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: `'string' | 'object'`,
      default: `undefined`,
      description: `Restrict conditional style values to one syntax. Flat values accept both the string form (bg="red hover:blue") and the object form (bg={{ base: 'red', hover: 'blue' }}); set this to keep one of them, which narrows the types, makes the runtime warn on the other in development, and tells tamagui generate-prompt and the skills to only show that form.`,
    },
    {
