...
```tsx
<Circle pin="top" centered size="$maxXl" />
```
...
```tsx
<SizableText size="$maxLg">Hello world</SizableText>
```

The Spread variant function will receive two arguments: the first is the value
given to the property (`"$maxXl"`), and the second is an object with
