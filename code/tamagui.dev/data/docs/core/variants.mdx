...
Beyond exact keys, a variant object can use type keys that match by the type of
the incoming value:
...
Exact keys always win: if a variant defines both `small: {...}` and
`string: (val) => ...`, passing `"small"` matches the exact key and any other
string falls through to the `string` resolver.
...
resolver variants, if you wanted to have a `pad` property that accepted all the
...
Resolver variants solve this problem. Instead, we can write:
...
Resolver variants save you from having to define hardcoded styles for every key
(`sm`, `md`, `lg`) in your token object. A resolver key matches every value in
its token scale, and props are typed from the key. The available names are:

- Token scales: `Size`, `Space`, `Color`, `Radius`, `ZIndex`
- Fonts: `FontSize`, `FontStyle`, `FontTransform`, `FontLineHeight`,
  `FontLetterSpacing`
- Themes: `Theme`
- Plain types: `number`, `string`, `boolean`, `any`

You can also combine names with `|` to accept several kinds of values with one
function:
...
Every functional variant receives two arguments: the first is the value given
to the property (`"lg"`), and the second is an object with
...
variants you need. Use the `any` key and it will grab all values that don't
match another key:
