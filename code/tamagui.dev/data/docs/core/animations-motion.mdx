...
<IntroParagraph>
  [Motion](https://motion.dev) combines JavaScript animation controls with the
  Web Animations API for smooth, browser-native animation.
</IntroParagraph>
...
The driver uses the Web Animations API (WAAPI), which runs animations on the
