...
Lightest bundle size, but lacks spring physics (easing curves only). Best for simple web-only apps.
...
No extra bundle hit beyond React Native Web, but runs on-thread. Good for basic cross-platform animations.
...
Wide support for both native and web with advanced features, but larger bundle size. Best performance on native platforms with off-thread animations.
...
Off-thread performance via WAAPI with excellent spring physics, medium bundle hit. Best for web-only apps that need smooth, physics-based animations.
