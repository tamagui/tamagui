...
keys by default. It is exported from `@tamagui/style`, `@tamagui/ui`, and
`tamagui`:
...
```tsx
import { splitStyleProps, stylePropsTextOnly } from '@tamagui/style'
...
```tsx
import { ClientOnly } from 'tamagui' // or '@tamagui/style'
...
```tsx
import { View, GetProps, styled } from '@tamagui/style'
...
```tsx
import { View, GetRef, styled } from '@tamagui/style'
