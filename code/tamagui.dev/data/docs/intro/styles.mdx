...
- **`hoverStyle`** - Styles applied when hovering over the element (web and native macOS/Windows; maps to CSS `:hover` on web)
...
pseudo modifiers are `hover` (web and native macOS/Windows), `press`, and `focus`.
