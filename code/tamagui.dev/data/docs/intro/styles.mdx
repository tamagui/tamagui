...
styled component: the [`html.*` primitives](/docs/core/html-primitives), the
base [View and Text components](/docs/core/view-and-text), or through the
...
For the full base styles, see the React Native docs (version 3 targets React
Native 0.82):
...
// Every style value may also be conditional, written as a string or an object.
// Both combine a base value with state, viewport, theme, platform, group, or
// container clauses:
//
//   backgroundColor="background hover:background-hover dark:gray-950"
//   backgroundColor={{ default: 'background', hover: 'background-hover', dark: 'gray-950' }}
//   padding="2 sm:4"
...
<MDXTabs id="style" defaultValue="styled">
<MDXTabs.List>
...
values are numbers, when the conditions stack up, or when you build the value in
code. Both compile to identical CSS.
...
v3 distinguishes raw numeric font sizes from exact px strings:
...
v3, preserving v2 visual output while making exact px intent explicit.
...
<html.button
  ...
  scale="press:0.98"
  outlineColor="focus:blue-500 focus-visible:blue-500"
...
For mount/unmount animations, use clauses on each animated property:

- **`enter:`** - Initial value when the component mounts (animates _from_ it)
- **`exit:`** - Final value when the component unmounts (animates _to_ it)
...
<html.div backgroundImage="linear-gradient(45deg, black 0%, white 50%, black 100%)" />

// filter with a CSS length
<html.div filter="blur(7px)" />

// On web: resolves to CSS custom properties
// boxShadow: "0 0 10px var(--t-shadow-shadow-color)"
...
```tsx
// Works on both web and native
<html.div border="1px solid borderColor" />
...
<html.span textShadow="2px 2px 4px shadow-color">
  Shadowed Text
</html.span>

// multiple shadows (web only)
<html.span textShadow="1px 1px 2px shadow-color, 0 0 8px color">
  Glowing Text
</html.span>
```
...
being one of: platform, screen size, theme, or group. They are modifiers in the
same flat value program as the base and state values.

For example you can target `dark:`, `ios:`, `group-hover/card:`, or a media
query such as `large:`. Every clause controls just the property carrying it.

#### Media query

Based on whatever media queries you define in `createTamagui`, use its name as
a modifier on native and web.
...
```tsx
<html.span color="ios:white" />
```

### Group

Groups let children target a parent interaction state:
...
Now only when the parent element is hovered the text will turn white. The allowed
...
declare module 'tamagui' {
  ...
  // If you want completion for named groups such as group-hover/card,
...
Now the text will be white, but only when the container matches the media query `sm`.
...
```tsx
<html.div group="card" container containerName="card">
  <html.div group>
    <html.span color="gray @sm/card:white @sm/card:group-hover/card:green" />
    <html.span color="gray @sm:white @sm:group-hover:green" />
  </html.div>
</html.div>
```
...
```tsx
<html.div group container untilMeasured="hide">
  <html.span color="gray @sm:white" />
</html.div>
```
