...
If you are installing through a coding agent, hand it the brief instead of doing
this by hand: [Set up with an agent](/docs/intro/agents) has a copy-paste prompt
for a new project and another for upgrading an existing one.
...
Tamagui takes advantage of new style features in React Native 0.81+ including
...
`@tamagui/style`, you can replace it with `tamagui`.

<Notice>

Web-only apps need `react-native` as a dev dependency. Native and universal apps
already have it. Nothing imports it at runtime on web, but Tamagui's prop types
reference React Native's, so without it you lose typing and autocomplete:

```bash
yarn add -D react-native
```

React Native ships its own types, so there's no separate `@types` package. In a
monorepo, put it in the app or the workspace root rather than a leaf package.
...
import { TamaguiProvider, html } from '@tamagui/style'
