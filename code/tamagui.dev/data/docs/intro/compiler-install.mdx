...
This only applies to web-only apps. Native and universal apps already have
`react-native` installed. For web-only apps, you still need `react-native`
types in the parent project or workspace root for prop typing and autocomplete.
You can either install `react-native` directly:
...
Tamagui's web runtime doesn't require `react-native` at runtime, but current
prop typing and autocomplete rely on the `react-native` types being present.
