...
import { setNativeStyleEngine } from '@tamagui/style'
...
export default () => <html.div disableOptimization />
```

### Web-only apps

Native and universal apps already have `react-native` installed. Web-only apps
need it as a dev dependency:

```bash
yarn add -D react-native
```

Nothing imports it at runtime on web, but Tamagui's prop types reference React
Native's, so without it you lose typing and autocomplete. React Native ships its
own types, so there's no separate `@types` package. In a monorepo, put it in the
app or the workspace root rather than a leaf package.
