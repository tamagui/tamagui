...
Accordion is already installed in `tamagui`, or you can install it
independently:
...
Wraps an `Accordion.Trigger`. Use the `asChild` prop to update it to the
...
Toggles the collapsed state of its associated item. It should be nested inside
of an `Accordion.Header`.
...
Adheres to the
[Accordion WAI-ARIA design pattern](https://www.w3.org/TR/wai-aria-practices-1.1/#accordion).
