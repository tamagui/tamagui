...
<Notice>
  **Portal vs modal prop:** These serve different purposes and are not redundant.
  `Dialog.Portal` controls **where** the dialog renders (to `document.body`),
  while the `modal` prop controls **how** it behaves (focus trapping, overlay, outside interactions).
  You need both for a fully functional modal dialog.
</Notice>
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: 'true',
      description: `Controls dialog behavior: when true, traps focus, shows overlay, and disables interaction with elements behind the dialog. When false, behaves more like a popover without focus trapping. Note: this controls behavior, not rendering location - use Dialog.Portal to control where the dialog renders in the DOM.`,
    },
    {
...
Renders Dialog content into a portal (to `document.body` on web, or a portal host on native). This handles **where** the dialog renders in the DOM, managing z-index stacking and ensuring the dialog appears above other content. The `modal` prop on Dialog controls **behavior** (focus trapping, overlay), while Portal controls **rendering location**.
...
Displays behind Content. Only renders when `modal={true}` (the default). Use `forceMount` if you need the overlay to render when `modal={false}`.
...
### Using Sheet with Adapt

When used with `Adapt`, Dialog can render as a sheet when that breakpoint is
active.

See [Sheet](/docs/components/sheet) for more props.

Must use `Adapt.Contents` inside the `Sheet.Frame` to insert the contents
...
<Notice>
  Note that Sheet currently doesn't preserve state of the contents when
