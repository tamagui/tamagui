...
Each name is a row in the Button's own size table (`xs` through `xl`,
  ...
  data={[
    {
      ...
      required: false,
      type: "'xs' | 'sm' | 'md' | 'lg' | 'xl'",
      description: `A named size ("xs" to "xl", default "md") is a row in the Button's size table: padding, border radius, font size, and icon size, never a height. Published to Button.Text and Button.Icon through the Button context.`,
    },
    {
      name: 'variant',
      required: false,
