...
Each name is a rung in the config sizing ladder (`xs` through `xl`,
default `md`), so the frame, its text, and its icon agree by construction.
See [Control Sizing](/docs/guides/sizing).
...
derived from the sizing ladder so it stays consistent with the button's
dimensions.
  ...
  data={[
    {
      ...
      required: false,
      type: "'xs' | 'sm' | 'md' | 'lg' | 'xl'",
      description: `A named size ("xs" to "xl", default "md") is a rung in the config sizing ladder: padding, border radius, font size, and icon size, never a height. Published to Button.Text and Button.Icon through the Button context. See the Control Sizing guide.`,
    },
    {
      name: 'variant',
      required: false,
