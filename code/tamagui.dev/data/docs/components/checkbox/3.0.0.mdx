...
[props](/ui/stacks), plus. Sizes come from the shared
  ...
  data={[
    {
      ...
      required: false,
      type: "'xs' | 'sm' | 'md' | 'lg' | 'xl'",
      description: `A named size ("xs" to "xl", default "md") is a rung in the sizing ladder: the box is the ladder's square derivation at the rung's radius.`,
    },
    {
      name: 'labeledBy',
      type: 'string',
