...
LinearGradient is a YStack that accepts all Tamagui style props as well as theme colors. On native it renders `expo-linear-gradient` inside an `absoluteFill` wrapper.
