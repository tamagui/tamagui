...
<IntroParagraph>
  RadioGroup lets users choose one option from a keyboard-navigable set on web
  and native.
</IntroParagraph>
...
Item sizes come from the shared [Sizing](/docs/core/sizing) ladder.
  ...
  data={[
    {
      ...
      required: false,
      type: "'xs' | 'sm' | 'md' | 'lg' | 'xl'",
      description: `A named size ("xs" to "xl", default "md") is a rung in the sizing ladder: the circle is the ladder's square derivation.`,
    },
    {
      name: 'labeledBy',
      type: 'string',
