...
AlertDialog is built on Dialog with stricter accessibility requirements, and [automatically stacks](/ui/z-index) above other content.
...
Contains every component for the AlertDialog. Shares all [Dialog Props](/ui/dialog#api-reference), except modal which is on by default. Adds:
...
Defaults to Paragraph, see [Paragraph](/ui/text#paragraph).
