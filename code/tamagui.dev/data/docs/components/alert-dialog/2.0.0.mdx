...
Alert Dialog is already installed in `tamagui`, or you can install it
independently:
...
In order to use this component independently of `tamagui`, you'll first need to
install the `@tamagui/portal` package:
...
Contains every component for the AlertDialog. Shares all
[Dialog Props](/docs/components/dialog#api), except modal which is on by
default. Adds:
...
Renders AlertDialog into appropriate container. Beyond
...
Main container for AlertDialog content, this is where you should apply
animations.
...
Closes the AlertDialog, accepts all YStack props. Recommended to use with your
own component and `asChild`.
