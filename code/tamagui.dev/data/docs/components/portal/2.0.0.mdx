...
<IntroParagraph>
  Portal sends content to another part of the rendered tree while preserving
  Tamagui theme and configuration context.
</IntroParagraph>

Portal is included in `tamagui` and is used by [Sheet](/ui/sheet),
[Dialog](/ui/dialog), [Popover](/ui/popover),
[Select](/ui/select), and [Toast](/ui/toast). See
[Stacking (zIndex)](/ui/z-index) for how overlay layering works.

## PortalProvider

`TamaguiProvider` from `tamagui` already wraps your app in a `PortalProvider`
with a root `PortalHost`, so most apps need no setup. Add one yourself only when
you render the bare `TamaguiProvider` from `@tamagui/core`:
...
export function App() {
  return (
    <TamaguiProvider config={config}>
      <PortalProvider shouldAddRootHost>{/* your app */}</PortalProvider>
    </TamaguiProvider>
  )
}
```

Render only one `PortalProvider` with a root host. Nesting a second one under
`TamaguiProvider` from `tamagui` mounts two root hosts and causes hydration
mismatches; pass `shouldAddRootHost={false}` if you need the nested provider.
  ...
  data={[
    {
      ...
      type: 'boolean',
      default: 'true',
      description: `Mounts a PortalHost at the end of the provider for portals to render into.`,
    },
    {
      name: 'rootHostName',
      type: 'string',
      default: `'root'`,
      description: `Name of the root PortalHost.`,
    },
  ]}
/>
