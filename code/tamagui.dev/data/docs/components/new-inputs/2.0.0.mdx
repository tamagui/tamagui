...
Using Web APIs and relying on bare Tamagui with no `react-native-web` dependency on web compared to old Input component, support scaling all the styles up or down using the `size` property, and full `theme` support.
