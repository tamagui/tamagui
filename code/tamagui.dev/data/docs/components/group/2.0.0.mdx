---
title: Group
description: Arrange controls in horizontal or vertical groups
name: group
component: Group
package: group
demoName: Group
...
<IntroParagraph>
  Group arranges related controls horizontally or vertically, squares off the
  border radius where items meet, and passes disabled state to its children.
</IntroParagraph>
...
```tsx
import { Activity, Airplay } from './components/icons' // your generated icons, see Lucide Icons
import { Button, XGroup } from 'tamagui'

export default () => (
  <XGroup>
    <XGroup.Item>
      <Button size="sm" height="gtSm:5" icon={Activity}>
        First
      </Button>
    </XGroup.Item>
    <XGroup.Item>
      <Button size="sm" height="gtSm:5" icon={Airplay}>
        Second
      </Button>
    </XGroup.Item>
  </XGroup>
...
`Group`, `XGroup` and `YGroup` extend [YStack](/ui/stacks), getting [Tamagui standard props](/docs/intro/props), plus:
