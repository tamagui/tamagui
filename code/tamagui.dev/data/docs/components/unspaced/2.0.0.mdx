...
While the `space` property is deprecated, Unspaced lives on in a better form,
one which is easily back-ported to add Unspaced support (as well as adding `gap`
support).
...
```tsx
import { Text, Unspaced, View } from 'tamagui'
...
[VisuallyHidden](/docs/components/visually-hidden), which avoids uses Unspaced
but also hides the contents in an accessible manner.
