...
A render prop component that provides placement-aware values for animations. Use this when you want animations that respond to the actual placement of the content (e.g., slide-in from the correct direction).

<PropsTable
  data={[
    {
      name: 'children',
      type: '(info: PlacementInfo) => React.ReactNode',
      required: true,
      description: 'Render function that receives placement information.',
    },
  ]}
/>

The render function receives a `PlacementInfo` object with:

- `side`: `'left' | 'right' | 'top' | 'bottom'` - The actual side the content is placed on
- `align`: `'start' | 'center' | 'end'` - The alignment within that side
- `placement`: The full placement string (e.g., `'bottom-start'`)
- `isLeft`, `isRight`, `isTop`, `isBottom`: Boolean helpers
- `xDir`: `1` for right-opening, `-1` for left-opening - useful for horizontal animations
- `yDir`: `1` for bottom-opening, `-1` for top-opening - useful for vertical animations
...
This is especially useful when `allowFlip` is enabled - the submenu might open on the left instead of right when near the viewport edge, and `xDir` will automatically adjust so your animation slides in from the correct side.
