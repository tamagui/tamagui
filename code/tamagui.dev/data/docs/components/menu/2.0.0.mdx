...
<IntroParagraph>
  Menu presents keyboard-navigable actions, selections, groups, and submenus in
  a popover or platform-native menu.
</IntroParagraph>
...
```bash
npm install @react-native-menu/menu react-native-ios-context-menu react-native-ios-utilities zeego sf-symbols-typescript
```
...
<Notice theme="yellow">
  **Expo Router users:** This import must run _before_ `expo-router/entry`. Create an `index.js` at your project root that imports the setup first, then expo-router, and update your `package.json` main field to `"index.js"`. See the [Expo guide](/docs/guides/expo) for details.
</Notice>
...
Trigger element for the menu. Accepts [Tamagui Props](/docs/intro/props).
  ...
  data={[
    {
      ...
      type: 'React.ReactNode',
      required: true,
      description: 'One or more Menu.RadioItem components.',
    },
    {
      name: 'value',
      type: 'string',
      required: false,
      description: 'The controlled value of the currently selected item.',
    },
    {
      name: 'onValueChange',
  ...
  data={[
    {
      ...
      type: 'string',
      required: true,
      description: 'Unique identifier for the radio item.',
    },
    {
      name: 'value',
      type: 'string',
