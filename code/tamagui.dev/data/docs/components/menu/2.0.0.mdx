...
Menu items use a `focus:` clause for highlighting rather than a `hover:` clause. This ensures a unified highlight experience when switching between mouse and keyboard navigation - only one item is ever highlighted at a time.
...
```jsx
// Default behavior uses the focus clause for highlight
<Menu.Item>
  <Menu.ItemTitle>Settings</Menu.ItemTitle>
</Menu.Item>

// Custom highlight styling uses focus, not hover
<Menu.Item backgroundColor="focus:blue5">
