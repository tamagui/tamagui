...
<IntroParagraph>
  Slider lets users set one or more values with pointer, touch, or keyboard
  input in horizontal and vertical layouts.
</IntroParagraph>
...
export default () => (
  <Slider size="md" width={200} defaultValue={[50]} max={100} step={1}>
    <Slider.Track>
      <Slider.TrackActive />
    </Slider.Track>
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      required: false,
      type: 'string',
      description: `For usage with forms. On web, renders one hidden input per thumb under this name, so multi-thumb values submit as repeated entries readable via FormData.getAll(name).`,
    },
    {
      name: 'value',
      required: false,
...
Slider.Track inherits `YStack`, extending all the default [props](/docs/intro/props).

### Slider.TrackActive

Slider.TrackActive inherits `View`, extending all the default [props](/docs/intro/props).

### Slider.Thumb

Slider.Thumb inherits `YStack`, extending all the default [props](/docs/intro/props), adding:
