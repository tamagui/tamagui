...
Slider comes as multiple components that ship with default styles and are
sizable. The `size` prop on `<Slider />` will automatically pass size down to
...
You can also optionally style any component, either using inline style props or
by wrapping with `styled`:
...
`Slider.Track` Inherits `SizableStack`, extending all the default
[props](/docs/intro/props).

### Slider.TrackActive

`Slider.Track` Inherits `Stack`, extending all the default
[props](/docs/intro/props).

### Slider.Thumb

`Slider.Track` Inherits `SizableStack`, extending all the default
[props](/docs/intro/props), adding:
