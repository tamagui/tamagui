...
For native press handling and the style runtime, see
[core Native](/docs/core/native).
...
uses a JS-based approach that breaks React context, or requires you to hoist
data awkwardly. With native portals, your custom contexts (navigation, app
state) work inside Sheet, Dialog, Popover, and other portaled content.
...
**Native menus** render platform-native context menus instead of custom
overlays. They use the OS menu API for haptics, blur effects, and system
integration.
...
[core Native](/docs/core/native#native-runtime) for setup.
