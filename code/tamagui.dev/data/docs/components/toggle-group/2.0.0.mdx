...
ToggleGroup is already installed in `tamagui`, or you can install it
independently:
...
`ToggleGroup` extends the [Group](/docs/components/group) component. You can
disable passing border radius to children by passing `disablePassBorderRadius`.
plus:
...
`ToggleGroup.Item` extend Stack views inheriting all the
...
When it is active, it will receive an `active` prop set to true. This means you
can customize the active styles like so:
