...
<IntroParagraph>
  ToggleGroup arranges two-state buttons into a keyboard-navigable single or
...
```tsx
import { ToggleGroup, XGroup, style } from 'tamagui'

const activeItemStyle = style({ backgroundColor: 'color-5' })
...
`ToggleGroup.Item` extends View inheriting all the [Tamagui standard props](/docs/intro/props), plus:
...
<PropsTable
  data={[
    ...
    },
    {
      name: 'activeStyle',
      type: 'StylePiece',
      description: `Style piece applied when the item is in its active/pressed state. Accepts a style() piece.`,
    },
    {
      name: 'activeTheme',
      type: 'string | null',
