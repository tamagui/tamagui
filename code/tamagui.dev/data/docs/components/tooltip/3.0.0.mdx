...
Used to trigger opening of the popover when uncontrolled, see YStack in [Stacks](/ui/stacks).

### Tooltip.Content

Renders as a YStack (see [Stacks](/ui/stacks)) with an extra `size` prop that accepts any `SizeTokens`.
