...
Tooltip is already installed in `@tamagui/ui`, or you can install it independently:
...
```tsx
import { Tooltip } from '@tamagui/ui' // or '@tamagui/tooltip'
...
<Notice>
  If using `modal={true}` (which is `true` by default), refer to the [PortalProvider installation](/ui/tooltip/2.0.0#portalprovider) for more information.
</Notice>
