...
Displays behind the animated sheet container. Extends [YStack](/docs/components/stacks).
...
Displays the visual sheet surface behind the container content. Extends [YStack](/docs/components/stacks).
  ...
  data={[
    {
      ...
      type: 'boolean',
      description: `Disables the default background extension below the sheet. Leave it omitted or false when a spring can overshoot on open; the background prevents page content from showing through below the sheet.`,
    },
  ]}
/>
