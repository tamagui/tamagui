...
<Notice>
  `when={true}` forces the adaptation on and skips the platform check entirely,
  so `<Adapt when={true} platform="touch">` is active on desktop web too. Use a
  media query string when you want both conditions to apply.
</Notice>
  ...
  data={[
    {
      ...
      required: false,
      description: `A media query key from your config, or a boolean. Active when the query matches. true forces the adaptation on and skips the platform check.`,
    },
    {
      name: 'platform',
      type: `'web' | 'native' | 'ios' | 'android' | 'touch' | null`,
      required: false,
      description: `Restricts the adaptation to a platform. Combined with a media query string, both must pass.`,
    },
    {
      name: 'scope',
      type: 'string',
...
With neither `when` nor `platform` set, Adapt is never active.
...
does not double up on it. Sheet declares all three when it is the target:
  ...
  data={[
    {
      ...
      type: 'boolean',
      required: false,
      description: `The container already scrolls its contents.`,
    },
    {
      name: 'overlay',
      type: 'boolean',
      required: false,
      description: `The container already renders a backdrop overlay.`,
    },
    {
      name: 'dismiss',
      type: 'boolean',
      required: false,
      description: `The container already handles dismissal.`,
    },
  ]}
/>

Read them back with `useAdaptedCapabilities()`.
