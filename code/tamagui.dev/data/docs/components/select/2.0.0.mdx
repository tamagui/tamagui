...
Use the `transition` prop to control the animation speed. The `select` animation is optimized for quick, snappy movement. You can also use other animation names from your config like `quick`, `quicker`, or `quickest`.
