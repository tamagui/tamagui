...
When used alongside `<Adapt />`, Select can render as a sheet when that breakpoint is active.
...
Must use `Adapt.Contents` inside the `Sheet.Frame` to insert the contents given to `Select.Content`
