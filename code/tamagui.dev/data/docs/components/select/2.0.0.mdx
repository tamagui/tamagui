...
An animated indicator that highlights the currently focused item. Place it inside `Select.Viewport` to enable the sliding indicator animation.
...
<Notice>
  To disable the indicator animation and use the default hover/press styles instead, simply remove `Select.Indicator`. The `Select.Item` component has `hoverStyle` and `pressStyle` with `$backgroundHover` and `$backgroundPress` by default.
</Notice>
