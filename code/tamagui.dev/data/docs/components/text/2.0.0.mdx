...
<Notice theme="green">
  The base `Text` component is already included in `@tamagui/style` (and `tamagui`). This
...
<Notice>
  The `numberOfLines` prop is supported on the base `Text` component from `@tamagui/style`
...
The base text component from `@tamagui/style`. Accepts all [Tamagui style props](/docs/intro/props) for text elements.
