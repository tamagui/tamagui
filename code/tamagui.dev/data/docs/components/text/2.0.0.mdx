...
<IntroParagraph>
  Text primitives connect semantic text, configured fonts, shared sizing, and
  Tamagui style props across web and native.
</IntroParagraph>
...
<Highlights
  features={[
    'Font-specific themes for spacing, weights, and sizes',
    'Automatic theme matching through the size prop',
    'Flat media, hover, press, and focus clauses',
  ]}
/>
...
export default () => (
  <Paragraph>
    ...
    <Span color="blue-10">colored span</Span>.
  </Paragraph>
)
```
  ...
  data={[
    {
      name: 'size',
      required: false,
      type: 'SizeTokens | true',
      description: `Sets font size, line height, letter spacing, and font weight from your theme's font configuration. Accepts any size token (e.g., "4") or true for the default size.`,
    },
    {
      name: 'ellipsis',
      required: false,
