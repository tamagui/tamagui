...
<Notice theme="yellow">
  This page documents the v2 imperative Toast API (`ToastProvider`,
  `ToastViewport`, `useToastController`, `useToastState`), which was removed in
  v3. Use the composable [Toast](/ui/toast) and the `toast()` function instead.
  See the [upgrade guide](/docs/guides/how-to-upgrade) for the migration.
</Notice>

<Highlights
  features={[
    `Automatic dismissal`,
    `Closing paused on hover, focus, window blur, and mobile touch`,
    `Swipe gesture dismissal`,
    `Support for Tamagui's animation drivers`,
    `Android/iOS native toasts and web Notification API`,
  ]}
/>
...
The portal for toasts to be directed to. Should be used inside [ToastProvider](#toastprovider). Beyond `View` props, adds:
...
Contains the Title, Description, Action and Close component. Should be used inside [ToastProvider](#toastprovider). Extends `View` and adds:
...
Should be used inside [Toast](#toast). Extends `View`. You can pass `asChild` to this component and use a custom `<Button>` inside.
