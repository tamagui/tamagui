...
<Notice theme="yellow">
  The v1 imperative Toast API documented on this page (`ToastProvider`,
  `ToastViewport`, `useToastController`, `useToastState`) was removed in v3. Use
  the composable [Toast](/ui/toast) and the `toast()` function instead. See the
  [upgrade guide](/docs/guides/how-to-upgrade) for the migration.
</Notice>
