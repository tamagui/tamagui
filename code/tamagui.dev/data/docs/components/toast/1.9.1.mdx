...
The portal for toasts to be directed to. Should be inside [ToastProvider](#toastprovider). Beyond `View` props, adds:
...
Contains the Title, Description, Action and Close component. Should be inside [ToastProvider](#toastprovider). Extends `View` and adds:
...
Should be inside [Toast](#toast). Extends `View`. You can pass `asChild` to this component and use a custom `<Button>` inside.
