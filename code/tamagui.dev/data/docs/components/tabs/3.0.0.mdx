...
<IntroParagraph>
  Tabs organizes related panels behind a controlled or uncontrolled,
  keyboard-navigable list of triggers.
</IntroParagraph>
...
Container for the trigger buttons. Supports scrolling by extending [Group](/ui/group).
...
<Notice>
  ...
  items](/ui/group#custom-components--nested-items) for workarounds.
</Notice>
...
Here is a demo with more advanced animations using [AnimatePresence](/docs/core/animations#animatepresence-and-exitstyle) and [Tabs.Tab](#tabstab)'s `onInteraction` prop.
