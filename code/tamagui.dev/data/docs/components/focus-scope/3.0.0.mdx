...
In v3, FocusScope renders a `display: contents` wrapper so it does not add a
layout box around its children.
...
Use `noFocus` to disallow focus entirely while the scope is active - focus can
land neither inside nor outside the scope. Any element that receives focus is
immediately blurred, so `document.activeElement` settles on `document.body`.
This takes precedence over `trapped` and auto-focus behavior:
...
Pass JSX children directly. Function-as-children was removed in v3 because
FocusScope now owns the focus wrapper and renders it as `display: contents`.
