...
<IntroParagraph>
  Popover positions focused content around a trigger and can adapt the same
  content into a Sheet on smaller screens.
</IntroParagraph>
...
Popover shows content only while its trigger is pressed, floating above the
current content. It [automatically stacks](/ui/z-index) above other content.

<Notice>
  Popovers are not a recommended pattern for mobile apps. Use Adapt to render
  them as a Sheet instead, or conditionally render native UI.
</Notice>
...
In v3, Popover uses the same Adapt handoff path as Dialog. Adapted content stays
mounted through the sheet exit animation and unmounts after the sheet reports
that it is fully hidden.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: '!hoverable',
      description: 'Disable opening when the trigger receives keyboard focus. Ordinary popovers open on click, Enter, or Space. Set false to open on focus; hoverable popovers enable this behavior by default.',
    },
    {
      name: 'resize',
...
Popover. Arrows extend YStack, see [Stacks](/ui/stacks).
...
<PropsTable
  data={[
    ...
    },
    {
      name: 'size',
      type: 'number',
      description: 'Size of the arrow in pixels.',
    },
    {
      name: 'offset',
      type: 'number',
...
see [Stacks](/ui/stacks).

### Popover.Content

Extends PopperContent which extends a YStack (see
[Stacks](/ui/stacks)). Used to display the content of the popover.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      required: false,
      type: `boolean`,
      description: `Disables part presence gating so the content stays mounted. Matches Dialog forceMount semantics.`,
    },
  ]}
/>
...
Renders as YStack, see [Stacks](/ui/stacks).
...
When used with [Adapt](/ui/adapt), you can render a Sheet when that breakpoint
