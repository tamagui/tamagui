...
<Notice>
  Note: Popovers are not a recommended pattern for mobile apps, and so aren't supproted on native. Instead you can use Adapt and render them as a Sheet, or just conditionally render them. We landed support for them at one point, but we need the community to contribute tests in order to support them for mobile again.
</Notice>
