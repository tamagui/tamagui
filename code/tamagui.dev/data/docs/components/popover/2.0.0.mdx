...
A render prop component that provides placement-aware values for animations. Use this when you want animations that respond to the actual placement of the content (e.g., slide-in from the correct direction when `allowFlip` causes the popover to flip sides).
...
<Popover>
  ...
  <Popover.Portal>
    <Popover.Placement>
      {({ xDir, yDir }) => (
        <Popover.Content
          enterStyle={{ scale: 0.9, opacity: 0, y: -10 * yDir }}
          exitStyle={{ scale: 0.95, opacity: 0, y: -5 * yDir }}
        >
          {/* content */}
        </Popover.Content>
      )}
...
- `placement`: The full placement string
