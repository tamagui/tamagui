...
<Highlights
  features={[
    ...
    `Automatic transform-origin for scale animations.`,
  ]}
/>
...
Popover.Content automatically calculates the correct CSS `transform-origin` based
on the current placement and arrow position. This means scale animations will
naturally "grow" from the anchor point:
...
>
  {/* Content scales from the arrow/anchor point */}
</Popover.Content>
```

The transform origin updates automatically if the popover flips to a different
placement due to viewport constraints.

#### CSS Custom Properties

Popover.Content exposes several CSS custom properties on web for advanced styling:

| Property | Description |
|----------|-------------|
| `--tamagui-popper-available-width` | Available width within viewport |
| `--tamagui-popper-available-height` | Available height within viewport |
| `--tamagui-popper-anchor-width` | Width of the trigger/anchor element |
| `--tamagui-popper-anchor-height` | Height of the trigger/anchor element |

These can be used to create content that matches the anchor size or responds to
available space:

```tsx
<Popover.Content style={{ width: 'var(--tamagui-popper-anchor-width)' }}>
