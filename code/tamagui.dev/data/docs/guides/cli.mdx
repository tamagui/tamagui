...
This works with any build tool - just run `tamagui build` before your build command:
...
**Important:** This is meant for production builds only. It rewrites files in-place, so don't run it during development. Use in your deployment pipeline.
...
{
  "scripts": {
    "prebuild": "tamagui check && tamagui build ./src",
    "build": "next build"
  }
}
```
...
{
  "scripts": {
    "build": "tamagui build --target both ./src && tsc"
  }
}
```
