...
simple, ready to use Button, you can use the [Tamagui Button](/ui/button)
itself directly.
...
export const ButtonFrame = styled(View, {
  ...
  variants: {
    size: styled.dynamic<SizeTokens>((name, { tokens }) => ({
      ...
      // scale the resolved token values to fit this component's proportions
      // these multipliers are a stylistic choice for this design system
      gap: tokens.space[name].val * 0.2,
      paddingHorizontal: getVariableValue(getSpace(name)) * 0.8,
    })),
  } as const,
...
const ButtonIcon = (props: { children: any }) => {
  ...
  const smaller = getVariableValue(getSize(size)) * 0.5
...
export default () => (
  <Button group="btn" bg="pink">
    <Button.Text color={{ default: 'blue', 'group-hover/btn': 'red' }}>
      Hello world
    </Button.Text>
  </Button>
)
...
export default () => (
  <Button group="btn" bg="pink">
    <Button.Text color="blue group-hover/btn:red">
      Hello world
    </Button.Text>
  </Button>
)
...
export default createTamagui({
  ...
  themes: {
    light: {
      background: '#ccc',
      'background-press': '#bbb', // darker background on press
      'background-hover': '#ddd', // lighter background on hover
      color: '#222',
    },
  },
...
const ButtonFrame = styled(View, {
  // displayName sets the React debugging identity
  displayName: 'Button',

  alignItems: 'center',
  flexDirection: 'row',

  // bare values look for theme first, then fall back to tokens
  backgroundColor: {
    default: 'background',
    hover: 'background-hover',
    press: 'background-press',
  },
...
[`styled.dynamic`](/docs/core/variants#dynamic-variants):
...
import { SizeTokens, GetProps, createStyledHOC, withStaticProperties } from '@tamagui/core'
import { Button } from './OurButton'

const SizeContext = createContext<SizeTokens>('md')

const ButtonFrame = createStyledHOC(
  Button,
  ({ size = 'md', ...props }: GetProps<typeof Button>) => {
    return (
...
const ButtonText = createStyledHOC(Button.Text, (props: GetProps<typeof Button.Text>) => {
...
While it's explained in [the styled docs](/docs/core/styled#createstyledhoc),
the short of it is: if you want a functional component that returns a styled
component _to then be able to be styled again_, you need `createStyledHOC`.
...
With the new Tamagui APIs, sharing some props between parent and child
components no longer means carefully adding a forwardRef, re-jigging the types,
and threading context by hand, only to break memoization or scoped values
anyway.
