...
const ButtonFrame = styled(View, {
  ...
  borderRadius: '$maxLg', // 8

  // and space tokens are used for all others
  paddingHorizontal: '$maxMd', // 25
})
...
```tsx
<ButtonFrame size="$maxLg">
  <ButtonText size="$maxLg" />
</ButtonFrame>
```
...
export default () => (
  <Button size="$maxLg">
    <Button.Text size="$maxLg">Hello world</Button.Text>
  </Button>
)
```
...
export default () => (
  <CustomButton>
    ...
    <CustomButton.Text size="$maxMd">(Smaller text)</CustomButton.Text>
  </CustomButton>
)
```
...
const SizeContext = createContext<SizeTokens>('$maxLg')

const ButtonFrame = Button.styleable(
  ({ size = '$maxLg', ...props }: GetProps<typeof OGB.ButtonFrame>) => {
    return (
