...
<IntroParagraph>
  Tamagui includes runtime and compiler tools for understanding how a component
  renders and where its styles came from.
</IntroParagraph>
...
the styled component display names and even the file numbers and components they are in
...
The React `displayName` is available on the component itself, rather than in
`staticConfig`.

### Classes generated

Tamagui generates helpful CSS classes. For components created with `styled()`
where a `displayName` is set like this:
...
The classnames `tm-paragraph` and `font_body` will be output to DOM.
