...
You can also use the `getTheme` callback to customize any theme at any level.
See the [Customizing Themes with getTheme](#customizing-themes-with-gettheme)
...
const themes = createThemeBuilder({
  getTheme: ({ theme, level }) => {
    // Add transparency to backgrounds at deeper levels
    if (level > 1) {
      return {
        ...theme,
        background: `${theme.background}f0`, // add alpha channel
      }
    }
    return theme
  },
...
const themes = createThemes({
  ...
  getTheme: ({ theme, name }) => ({
    ...theme,
    // Add semantic tokens based on theme
    success: name.includes('dark') ? '#4ade80' : '#16a34a',
    error: name.includes('dark') ? '#f87171' : '#dc2626',
  }),
})
```
...
```tsx
const themes = createThemeBuilder({
  getTheme: ({ theme, level, scheme }) => {
    const customizations: Record<string, string> = {}

    // Level 1: base themes
    if (level === 1) {
      customizations.elevation = '0px'
    }
    // Level 2: surface themes
    else if (level === 2) {
      customizations.elevation = '2px'
      customizations.borderRadius = '8px'
    }
    // Level 3+: card/component themes
    else {
      customizations.elevation = '4px'
      customizations.borderRadius = '12px'
    }
...
This feature makes it easy to apply consistent customizations across your entire
theme suite without manually editing each theme.
