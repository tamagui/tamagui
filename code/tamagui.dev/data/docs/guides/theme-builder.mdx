...
```tsx
import { createV5Theme, defaultChildrenThemes } from '@tamagui/themes/v5-builder'
...
Use the `inverse` sub-theme when you want a subtree to use the opposite light or
dark palette:
...
<Theme name="inverse">
  <Card>Inverted colors</Card>
</Theme>
```

The default v3 config generates concrete inverse sub-themes from the same model
as `accent`: light parents use the dark palette, and dark parents use the light
palette. Because `inverse` is a real named sub-theme, the server can render the
final theme class and hydrate it without a client-only theme swap.

If you are building custom themes, generate inverse sub-themes by swapping the
palettes in your theme definition:

```tsx
inverse: [
  { parent: 'light', palette: 'dark', template: 'base' },
  { parent: 'dark', palette: 'light', template: 'base' },
]
```
