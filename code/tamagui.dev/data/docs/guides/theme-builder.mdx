...
- `createThemeSuite` makes generating a slightly opinonated suite of themes much simpler.

We've also released [Theme](/theme), a free point-and-click way to create themes that generates copy-paste `createThemeSuite` code for you.

## createThemeSuite

At it's simplest, `createThemeSuite` is as simple as:

```tsx
import { createThemeSuite } from '@tamagui/theme-builder'

export const themes = createThemeSuite({
  base: {
    palettes: {
      dark: ['#000', '#fff'],
      light: ['#fff', '#000'],
...
Under the hood, `createThemeSuite` uses `createThemeBuilder`. It's a very powerful way to generate nested themes, using a chainable API and generating strong types for you. But, it's also quite involved! 99% of projects should start with either hand-made, simple themes, or `createThemeSuite`.
