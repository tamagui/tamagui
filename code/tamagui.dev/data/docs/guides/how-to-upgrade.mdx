...
| `'...size'` / `'...fontSize'` spread variant keys | `styled.dynamic<T>((value, env) => ({ ... }))` |
| `':string'` / `':number'` / `':boolean'` type-key variants | a typed `styled.dynamic<string \| number \| boolean>(...)` |
| `'...'` catch-all variant key | `styled.dynamic<YourValue>(...)` with an explicit value type |
...
Search for those keys and replace the functional branch with one branded
dynamic, which types the prop directly:
