...
<Notice theme="blue">
  ...
  Config v6, React 19, React Native 0.81+, and TypeScript 5+ baseline.
</Notice>
...
### Config and theme entries removed

V3 ships one config generation lineage: v6. The v5 entries and every earlier
entry are gone. V2 applications that must retain the v5 theme builder should
stay on Tamagui v2. The v3 config does not bundle an animation driver, so import
one alongside it.

| Removed entry | Replacement |
| --- | --- |
| `@tamagui/config` (root, `config` / `configWithoutAnimations`) | `defaultConfig` from `@tamagui/config/v6` |
| `@tamagui/config/v3`, `/v4`, or `/v5` | `defaultConfig` from `@tamagui/config/v6` |
| `@tamagui/config` `animations` / `reanimated` and all `v5-*` animation entries | `@tamagui/config/animations-css`, `animations-rn`, `animations-reanimated`, or `animations-motion` |
...
| old `@tamagui/themes` roots and all `/v5*` entries | static `themes` and `tokens` from `@tamagui/themes` |
...
```bash
rg "@tamagui/theme-builder|defaultComponentThemes|themes/v5|config/v[3-5]|createV5Theme|componentThemes|grandChildrenThemes|surface[1-4]|color12"
```
...
<TamaguiView backgroundColor="blue5">
