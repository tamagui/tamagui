...
row in the component's own size table, never a height: it picks a font size,
...
a name (or `true` for the default), and each component owns its table inline, so
`size="4"` on a control is a type error. Numbers stay geometry on shapes
