...
V3 ships one config generation lineage: v5 and v6. Every entry from v4 and
earlier is gone, including the original unversioned `@tamagui/config` and
`@tamagui/themes` roots. Pick v5 if you want the v2-era token and type scales,
v6 if you want the Tailwind-aligned defaults. Neither entry bundles an
animation driver, so import one alongside it.

| Removed entry | Replacement |
| --- | --- |
| `@tamagui/config` (root, `config` / `configWithoutAnimations`) | `@tamagui/config/v5` (`defaultConfig`) or `@tamagui/config/v6` |
| `@tamagui/config/v3` (`config`) | `@tamagui/config/v5` (`defaultConfig`) or `@tamagui/config/v6` |
| `@tamagui/config/v4` (`defaultConfig`) | `@tamagui/config/v5` (`defaultConfig`) or `@tamagui/config/v6` |
| `@tamagui/config` `animations` / `@tamagui/config/reanimated` | `@tamagui/config/v5-css`, `v5-rn`, `v5-reanimated`, or `v5-motion` |
| `@tamagui/config/v3` and `/v4` `fonts` | `fonts` from `@tamagui/config/v5`, or build your own with `createSystemFont` |
| `@tamagui/config/v3` and `/v4` `media` / `breakpoints` | `media` from `@tamagui/config/v5`, which already spells the max-queries kebab-case (`max-md`) |
| `@tamagui/config/v3` `shorthands` | `shorthands` from `@tamagui/config/v5` (same set as v4) or `/v6` |
| `@tamagui/config` `createGenericFont` | `createSystemFont` from `@tamagui/config/v5` |
| `@tamagui/themes` (root: `themes`, `tokens`, `palettes`, `templates`, `shadows`, `componentThemeDefinitions`) | `@tamagui/themes/v5`; the builder pieces live in `@tamagui/theme-builder` |
| `@tamagui/themes/v3` | `@tamagui/themes/v5` |
| `@tamagui/themes/v3-themes` (and the Radix legacy color re-exports) | `@tamagui/themes/v5`, or `@tamagui/colors` for the color ramps |
| `@tamagui/themes/v4` (`defaultThemes`, `tamaguiThemes`, `tokens`) | `@tamagui/themes/v5` (`themes`, `tokens`), or `@tamagui/themes/v5-subtle` |
