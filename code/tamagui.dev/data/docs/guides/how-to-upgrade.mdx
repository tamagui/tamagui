...
### Style compatibility, line height, and position

v5 changes a few style defaults:

- `styleCompat` now defaults to `'web'`
- raw numeric `lineHeight` follows CSS unitless semantics on web
- `flex` follows CSS shorthand behavior in `'web'` mode
- `position` changes from `relative` to `static` (browser default)

If your layout relies on the old v4 behavior, restore it:
...
If you want React Native-style raw numeric `lineHeight` and Yoga flex behavior,
use:
...
In the default `'web'` mode, use strings for absolute line heights:

```tsx
<Text lineHeight="24px" />
```

Raw numbers stay useful for CSS multipliers:
...
token value. You may also need to add `position="relative"` explicitly on
containers that have absolutely-positioned children, and add
`flexBasis="auto"` where needed.
