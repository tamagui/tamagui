...
### `themeInverse` / `<Theme inverse>` to `theme="inverse"`
...
// after
<Button theme="inverse">Primary</Button>
<Theme name="inverse"><Card>...</Card></Theme>
```
...
- `Tabs.Trigger` removed in favor of `Tabs.Tab`
...
Move visual surface props from `Sheet.Frame` to `Sheet.Background`, and keep
layout props on `Sheet.Container`. `Sheet.Background` now owns
`disableHideBottomOverflow`.

`Sheet.Container` no longer clips content with `overflow="hidden"`. If you rely
on full-bleed content being clipped to the sheet's top radius, add clipping to
your content or container explicitly.

`Sheet.Overlay` must be a direct child of `Sheet`. In development, nested
overlays warn with:
...
- [ ] Replace `themeInverse` / `<Theme inverse>` with `theme="inverse"`
