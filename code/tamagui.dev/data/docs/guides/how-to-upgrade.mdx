...
| `@tamagui/get-button-sized`| Copy the `size` variant from the skin component (`tamagui/button` and friends); each component owns its table inline, see §5. |
...
rung in the config sizing ladder, never a height: it picks a font size,
...
The default is `md`. Control height is derived from the rung at build time, not
tabled per component. See [Control Sizing](/docs/guides/sizing).

Under `@tamagui/config/v6` the tables mirror shadcn:
...
as is the v2 spelling `size="$4"`. Rename those to the matching name
(`"$4"` is usually `md`). Numbers stay geometry on shapes
(`Square`, `Circle`, `Avatar` take size tokens) and font keys on text and icons.
...
no size styles at all. The old size variants were functions that happened to
absorb `undefined`; the named-size tables do not, so core now treats an
explicit `undefined` the way React `defaultProps` do: as absent. This applies to
every styled component and every `createStyledContext` provider, not only
`size`. To deliberately turn a default variant off, pass `null`:

```tsx
<Button size={maybeSize} />   // undefined falls back to md
<Button size={null} />        // no size table applied
```

### Smaller size-related removals

- `Checkbox`, `RadioGroup` and `Switch` now step through five real sizes
  (17, 20, 22, 25 and 28px tall). In the first betas `sm`, `md` and `lg` all
  rendered at 22px.
- `Tooltip` no longer accepts `size`. It only ever set the arrow px for
  `TooltipSimple`, which keeps `size` for that purpose.
- `@tamagui/ui` no longer exports a `ListItemIcon` component. The behavior
  package publishes `color` through `ListItemContext`; the icon px is the
  skin's decision (the `tamagui` ListItem still ships `ListItem.Icon`). The
  `ListItemIconProps` type stays exported.
- The unstyled `Slider` and `Slider.Thumb` take `size` as a px number or a font
  size key. Skin size names (`sm`, `md`) belong to the styled `tamagui` Slider,
  which resolves them itself.
- `useToggleGroupItem().color` is `undefined` when no color was set, where it
