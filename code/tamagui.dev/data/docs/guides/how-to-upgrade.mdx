...
<IntroParagraph>
  ...
  and gives every adaptive overlay the same composition model.
</IntroParagraph>
...
<CopyAgentSetupButton type="upgrade" my="2" />
...
```tsx
<View opacity={{ default: 1, hover: 0.7, sm: 0.8 }} />
```
...
| --- | --- |
| `marginHorizontal` | `marginInline` (`mx` also works) |
...
On native, `direction` maps to `writingDirection` and `verticalAlign` maps to
`textAlignVertical`, so web-authored styles apply on both platforms.
