...
the design. It preserves `$blue10` as `blue10`, so search for these names and
review them manually when adopting the v6 config:
...
Component `size` still takes size tokens. The styled `tamagui` components
(`import { Button } from 'tamagui/button'`) resolve `size` through your token scales:
frame height, padding, and radius come from size, space, and radius tokens, while
text and icon sizing come from the active font scale. Omitting `size` (or passing
`true`) resolves through the opt-in policy in `@tamagui/size`, so component
defaults no longer depend on global Tamagui settings:

```tsx
// styled component: pass a size token, or omit for the component default
import { Button } from 'tamagui/button'
