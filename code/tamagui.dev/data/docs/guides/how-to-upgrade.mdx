...
| `@tamagui/get-button-sized`| Use `resolveTokenSize` from `@tamagui/size` (also exported from `tamagui`): its `frame` result gives the size/space/radius trio. |
...
frame height/padding/radius from the size/space/radius tokens and text/icon
sizes from the active font scale. Omitting `size` (or passing `true`) resolves
through `settings.defaultSize` (and `settings.defaultTokens` for the per-category
defaults), so the whole kit shares one default control size:
