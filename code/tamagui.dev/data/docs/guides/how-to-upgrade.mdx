...
`true`) resolves through `settings.defaultSize` and `settings.defaultTokens`, so
the whole kit shares one default control size:

```tsx
// styled component: pass a size token, or omit for settings.defaultSize
