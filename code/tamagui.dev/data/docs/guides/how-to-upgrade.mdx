...
does not contain duplicate `@tamagui/style`, `@tamagui/core`, `@tamagui/web`, or
`tamagui` copies.

### `@tamagui/core` is now `@tamagui/style`

The package formerly published as `@tamagui/core` is named `@tamagui/style` in v3.
Nothing about it changed apart from the name.

You do not have to rename anything to upgrade. `@tamagui/core` still publishes, as
an alias that re-exports `@tamagui/style` for every entry it had before, including
`@tamagui/core/reset.css` and `@tamagui/core/native`. Both specifiers resolve to one
module instance, so mixing them in one app does not give you two copies of the
runtime, and the compiler optimizes either one.

New code should import `@tamagui/style`:
...
- `Sheet.Background` owns visual surface props like `bg`, `borderRadius`,
  `shadow*`, and decorative absolute layers.
...
```tsx
import { splitStyleProps } from '@tamagui/style'
...
```tsx
import { getExpandedShorthand } from '@tamagui/style'
...
while components imported from `tamagui` or `@tamagui/style` accept Tamagui
style props.
