...
<IntroParagraph>
  Build a shared component package that keeps its types, themes, and compiler
  optimizations in every app that consumes it.
</IntroParagraph>
...
```tsx
import { GetProps, type SizeTokens, YStack, styled } from 'tamagui' // or '@tamagui/core' if extending just that
