...
const getCircleSize = (size, { tokens }) => {
...
<Notice theme="yellow">
  v3 removed the runtime spread and type-key variant lookups such as `'...size'`,
  `':number'`, and `':string'`. Match variant values with the explicit type keys
  `number`, `string`, `boolean`, and the token-scale keys like `Size`, `Space`,
  `Color`, and `Radius` instead. A single function can back several keys, as
  `getCircleSize` does above.
</Notice>
