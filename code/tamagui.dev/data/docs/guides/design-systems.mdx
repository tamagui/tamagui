...
`@tamagui/style`, which contains simple base-level components.
...
If you want to build more from scratch, then use `@tamagui/style` and only import
...
`tamagui` or `@tamagui/style` as well. Let's create a Circle component:

`Circle.tsx`:

```tsx
import { GetProps, type SizeTokens, YStack, styled } from 'tamagui' // or '@tamagui/style' if extending just that

// one function drives both the numeric and the size-token cases
const getCircleSize = styled.dynamic<SizeTokens | number>((size, { tokens }) => {
