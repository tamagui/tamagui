		...
		try {
			...
			if (!subscriptionStatus.pro) {
				...
				if (isV2) {
					...
					console.log(
						"[Payment] V2 subscription response:",
						response.status,
						data,
					);
			...
			if (selectedPrices.chatSupport || selectedPrices.supportTier !== "chat") {
				...
				if (!upgradeResponse.ok) {
					...
					const upgradeError =
						upgradeData.error || "Failed to add Support/Chat";
					const addonType = selectedPrices.chatSupport
						? "Chat Support"
						: "Support Tier";
						...
						<YStack gap="$4" mb="$4">
							<YStack gap="$2">
								...
								<Paragraph size="$3" color="$color9">
									Enter your project name and primary domain. Your license
									covers this domain plus iOS/Android apps.
								</Paragraph>
							</YStack>
						...
						<XStack justify="space-between">
							...
							<YStack items="flex-end">
								...
								<Paragraph fontFamily="$mono">
									$
									{Math.ceil(
										calculateDiscountedAmount(yearlyTotal / 12, finalCoupon),
									)}
									/month
								</Paragraph>
							</YStack>
						...
						<XStack justify="space-between">
							...
							<YStack items="flex-end">
								...
								<Paragraph fontFamily="$mono">
									$
									{Math.ceil(
										calculateDiscountedAmount(teamSeats * 100, finalCoupon),
									)}
									/year
								</Paragraph>
							</YStack>
