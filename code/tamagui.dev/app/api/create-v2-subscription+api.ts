...
export default apiRoute(async (req) => {
	...
	try {
		...
		const paymentIntent =
			paidInvoice.payment_intent as Stripe.PaymentIntent | null;
		...
		const supportPriceId = supportTier
			? getSupportTierPriceId(supportTier)
			: null;
		if (supportPriceId) {
			try {
...
async function rollbackPayment(
	...
	try {
		...
		if (upgradeSubscription) {
			...
			console.info(
				`Rolled back upgrade subscription: ${upgradeSubscription.id}`,
			);
		}
