...
/**
 * The fixture's Metro Babel transformer.
 *
 * It carries its presets inline instead of a root babel config: a root
 * `babel.config.*` would also switch the Next build in this same fixture off
 * SWC and onto its Babel loader. Modules are deliberately left as ESM so
 * Metro's own `experimentalImportSupport` handles them and the Tamagui compiler
 * can link imports.
 */
const presets = [
