...
const receipt = () =>
  JSON.parse(readFileSync(path.join(root, '.tamagui-zero/vite-zero.graph.json'), 'utf8'))
...
test('loading the island does not create a second React instance', async ({ page }) => {
  ...
  const sameInstance = await page.evaluate(
    () =>
      (globalThis as any).__zeroReact ===
      (globalThis as any).__tamagui_island_runtime__.react
  )
