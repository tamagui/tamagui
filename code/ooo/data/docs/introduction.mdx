...
One lets you target both [React Native](https://reactnative.org) and web with a single codebase, replacing both Metro and a separate web framework with a single framework, with shared file-system routing. 
