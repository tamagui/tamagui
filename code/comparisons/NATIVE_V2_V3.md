...
The native group gate uses a forced press state on a real group parent and a styled native child. V2 expresses the child rule as a nested `$group-row-press` object, while V3 uses `group-press/row`. This is required by the checked-in V2 API contract: group selectors hold a style object, and its native style splitter explicitly skips the hover pseudo. The expected resolved child background is `#2563eb`; the inactive `#60a5fa` value does not satisfy the gate. V2 calls the imported component `Button`, while V3 calls it `ButtonFrame`, so the gate asserts those exact version-specific names before emitting the common `Button` identity used by the cross-version signature comparison.
