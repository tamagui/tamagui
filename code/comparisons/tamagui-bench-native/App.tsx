    ...
    for (let i = 0; i < ITEM_COUNT; i++) {
      arr.push(
        <View
          ...
          margin={1}
          opacity="enter:0"
          scale="enter:0.5 hover:1.1 press:0.95"
        />
      )
    }
    return <>{arr}</>
...
// themed = simple shape with a theme token bg. on the RUNTIME path (no compiler)
// this rides full createComponent with the theme hook active — the runtime cost
// of a themed element, the comparison point for the compiled _withStableStyle gear.
