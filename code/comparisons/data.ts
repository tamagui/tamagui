  ...
  // ═══════════════════════════════════════
  {
    ...
    utilities: [
      ...
      },
      {
        ...
        notes: 'Tamagui: `$visibility` exists via RN-web on web only; on native there is no visibility, idiomatic Tamagui uses `$o={0}` (opacity) or conditional render. NativeWind `invisible` maps to opacity:0 on native (collapse is web-only).',
      },
      {
        name: 'float',
      ...
      },
      {
        ...
        notes: 'Tamagui: `$objectFit` is web-only; on native use the Image component `resizeMode`/`objectFit` prop directly. NativeWind v5 maps object-* to the Image objectFit prop, so it works on native too.',
      },
      {
        ...
        support: { tamagui: 'web-only', tailwind: 'web-only', nativewind: 'partial', uniwind: 'none' },
        notes: 'Tamagui: `$objectPosition` web-only. NativeWind partially maps object-position on native (limited values).',
      },
    ],
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    utilities: [
      ...
      },
      {
        ...
        notes: 'Tamagui has no `$motion-reduce` style key; honor it imperatively (read the reduce-motion setting and swap the `animation` prop / disable animations). NativeWind v5 maps motion-reduce: to `prefers-reduced-motion` on web and RN AccessibilityInfo.isReduceMotionEnabled on native.',
      },
    ],
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    utilities: [
      ...
      },
      {
        ...
        notes: 'No Tamagui style key for safe areas; use the `useSafeAreaInsets()` hook and feed values into `$pt`/`$pb` (one extra line). NativeWind and Uniwind both ship first-class safe-area utilities; Uniwind also has `-safe-or-*` / `-safe-offset-*` variants.',
      },
    ],
  },
    ...
    utilities: [
      {
        ...
        notes: 'NativeWind v5 maps fill-* to the react-native-svg `fill` prop (colors resolve, incl. /N opacity), so it works on native. Tamagui has no SVG className utilities — pass `fill`/`stroke` props directly to @tamagui/lucide-icons or react-native-svg.',
      },
      {
        name: 'stroke',
        ...
        support: { tamagui: 'none', tailwind: 'web-only', nativewind: 'full', uniwind: 'none' },
        notes: 'NativeWind v5 maps stroke-* to the react-native-svg `stroke` prop on native. Tamagui: pass `stroke` directly to the icon/SVG component.',
      },
      {
        name: 'stroke-width',
        ...
        support: { tamagui: 'none', tailwind: 'web-only', nativewind: 'full', uniwind: 'none' },
        notes: 'NativeWind v5 maps stroke-width to react-native-svg `strokeWidth` on native.',
      },
    ],
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    utilities: [
      ...
      },
      {
        ...
        support: { tamagui: 'web-only', tailwind: 'web-only', nativewind: 'partial', uniwind: 'none' },
        notes: 'Tamagui: `$caretColor` is registered only for the web target. On native, RN exposes caret color via the TextInput `cursorColor` (Android) / `selectionColor` props, not a style prop — NativeWind partially bridges this.',
      },
      {
        name: 'accent-color',
    ...
    utilities: [
      {
        ...
        support: { tamagui: 'none', tailwind: 'web-only', nativewind: 'partial', uniwind: 'none' },
        notes: 'NativeWind v5 `sr-only` only emits the visual-hide style on native (1x1, absolute, clipped) — it does NOT wire accessibilityElementsHidden / importantForAccessibility / aria-hidden, so the semantics differ from web. `not-sr-only` is web-only. Tamagui has no sr-only utility; use the RN accessibility props (`accessible`, `aria-hidden`, `accessibilityLabel`) directly.',
      },
      {
  ...
  // ═══════════════════════════════════════
  {
    ...
    utilities: [
      ...
      },
      {
        ...
        examples: {
          tamagui: '$bg="$blue10" / $bg="rgba(0,0,255,0.5)"',
          tailwind: 'bg-blue-500/50 text-black/75',
          nativewind: 'bg-blue-500/50 text-black/75',
          uniwind: 'bg-black/50',
        },
        support: { tamagui: 'partial', tailwind: 'web-only', nativewind: 'full', uniwind: 'full' },
        notes: 'Tamagui has no inline `/N` modifier — use a theme alpha token (`$color05`, `$blue10`) or an explicit rgba()/hsla() value. NativeWind v5 fully supports `/N` cross-platform (Tailwind v4 compiles it to color-mix(); v5 ships a runtime color-mix resolver). Uniwind supports `/N` in its examples.',
      },
    ],
  },
]
