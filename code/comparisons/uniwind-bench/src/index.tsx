...
const heavyUniwindColors = ['bg-blue-500', 'bg-green-500', 'bg-pink-500', 'bg-orange-500']

function HeavyItems({ seed }: { seed: number }) {
  return useMemo(() => {
    const arr = []
    for (let i = 0; i < HEAVY_COUNT; i++) {
      const color = heavyUniwindColors[(i + seed) % 4]
      arr.push(
        <View
...
// ── scenario 5: animated (no hover in uniwind) ───────
