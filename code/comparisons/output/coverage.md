...
| **transition-property** | ⚠️ | 🌐 | ⚠️ | ⚠️ | **⚠️ Tamagui, NativeWind v5, Uniwind:** Tamagui has no `transition-*` class; instead any prop change on a component with a transition driver animates automatically (cross-platform). NativeWind v5 transition-* is experimental on native (Reanimated-backed, RN 0.81+ / New Arch). Uniwind transitions are Pro-tier. |
