/**
 * Uniwind native bench harness. Same shape as tamagui-bench-native/App.tsx but uses
 * className on RN View via uniwind. Deep-link routed:
 *   exp://HOST/--/?case=<scenarioId>&n=<counter>
 *
 * Note: uniwind on native doesn't support hover/group-hover yet, so those scenarios
 * record the no-interaction static styling cost only.
 */
import 'uniwind'
  ...
  return useMemo(() => {
    ...
    for (let i = 0; i < ITEM_COUNT; i++) {
      arr.push(
        <View
          key={i + seed * ITEM_COUNT}
          className="flex-row items-center gap-2 p-2 rounded-lg bg-gray-100 m-px"
        >
    ...
    for (let i = 0; i < HEAVY_COUNT; i++) {
      ...
      arr.push(
        <View
          ...
          className="flex-row items-center gap-3 p-3 rounded-xl border border-gray-200 mb-1"
        >
