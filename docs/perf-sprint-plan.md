...
**Native bench harness — shipped.** `code/comparisons/run-benchmarks-native.ts` drives an iOS sim via Expo Go. Three sister apps share the same 5-scenario shape as the web bench: `tamagui-bench-native/`, `nativewind-bench-native/` (NativeWind v5 preview + Tailwind v4), `uniwind-bench-native/` (Tailwind v4). Each app reads `?case=<id>` from a deep-link, mounts the scenario, times mount + rerender with `performance.now()` inside `useLayoutEffect`, POSTs to the orchestrator's HTTP server at `:8091`. Orchestrator boots the sim (picks one with Expo Go installed), starts metro per-framework, sequentially deep-links the 5 scenarios, kills metro, moves to the next framework. Outputs `output/benchmarks-native.json` (always) + `output/benchmarks-native.html` (with `--html`). First-run baseline (1 run, 200 items, 60 for heavy, iPhone 16 Pro sim): Tamagui (runtime) is 2-4x slower than NativeWind v5 and Uniwind on every scenario, with the group-hover gap at 2.5x (256ms vs 100-117ms) — matching the sprint plan's expectation that native compiler work (track #3, pseudo states + styled() definitions) is the largest available native win. Android adds matrix complexity, deferred.
