Here's a fully built tamagui config v4 just without the themes because they are
quite large.

Config v4 uses CSS animations by default on web and react-native animations on
mobile.
...
So you just use that size token like so (see tamagui.mdx in library for more on
definining and using tokens):
