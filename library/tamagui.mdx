Tamagui is a style library `@tamagui/style`, and a separate UI kit `tamagui`. It
...
`@tamagui/metro-plugin` for Metro.

Note `tamagui` is a superset of `@tamagui/style`, so you can use that in general
for code generation.
...
```tsx
import { createTamagui, getConfig } from '@tamagui/style'
...
// get typescript types on @tamagui/style imports:
...
Tamagui themes are special and can do a lot, just know that you can generate
them a few ways.
...
And it generates a whole bunch of themes for you that are very nice like `light`, `light_accent`, `light_accent_Button`, etc. For more, see the Config v5 docs at /docs/core/config-v5.
