Tamagui is a style library `@tamagui/core`, and a separate UI kit `tamagui`. It also has an optional optimizing compiler `@tamagui/compiler` that generally you only use through it's bundler plugins like `@tamagui/vite-plugin` or `@tamagui/babel-plugin` for Metro.
