...
- Sheet.Frame => Sheet.Container + Sheet.Background (rewritten by @tamagui/codemod-flat-values). Container no longer clips with overflow hidden. Sheet.Overlay must be a direct child of Sheet.
...
- release-gate investigation: remove `TamaguiProvider` and make `createTamagui(...)` the only app setup, with exactly one global Tamagui config. Determine whether a `<TamaguiRoot />` can own portal attachment and the other root-level behavior, then validate the design across web, native, SSR, and library boundaries before implementation. If it holds up, ship it before the v3 release. Related design: `plans/v3-single-config-loading.md`.
