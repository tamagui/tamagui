...
2. **No `themes` map.** Theme targeting is the value grammar's own theme
   ...
   subtree (`hover:`, `sm:`) are a dev error.
...
- **Theme modifiers here are broader than the style grammar's.** `getCondition`
  ...
  name segment instead (the rule the `themes` map already used), so `blue:`
  works. Whether the style grammar should widen to match is open, and worth
  answering: under the current rule, theme modifiers in style props are close
  to useless for any sub-theme.
