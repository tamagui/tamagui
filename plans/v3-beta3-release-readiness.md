...
| Fixed locally, push pending | this session | Checks on `a7bf975a27` has four failures, three deterministic and one load-dependent. `checks`: `@tamagui/native-registry` declared `typescript@^5.9.2` against a repo standard of `~5.9.2`, so manypkg failed the job for every branch. `unit-tests`: the `codemod-flat-values` kitchen-sink corpus gate asserts zero v1 conversion sites and found 3, all in the new `NativeRegistry{Bench,Parity,Showdown}Case` fixtures. `integration-tests (1/3)`: 7 default-project Playwright tests carried pre-v6 expectations, 4 colors (`color9`, `shadow-color`, `light_green` background, `light_blue` `color`/`color10`) and 3 selectors invalidated by the skin restructure and component-theme removal (`.is_Checkbox`, `.is_Toggle`, `.is_SliderActive`). `integration-tests (3/3)`: two reanimated tooltip suites compared distance between consecutive `requestAnimationFrame` samples, which cannot tell a teleport from a starved frame; the same shard passed on the PR event and failed on the push event at the same SHA. | Fixed in six local commits on `v3-beta`. Each expected color was recomputed from `@tamagui/config/v6` and the theme builder rather than read off the render. The tooltip metric is restored to the velocity form from `e5f495b9f0`, now shared by both suites through `tests/utils.tsx` and applied to `TooltipStaticClobber`, which never had it; it keeps the same 150 px per 60 Hz frame boundary, keeps the synthetic 151 px negative control, and passed 15 of 15 repeats at 17% CPU idle. Local receipts: `bun run check` green, kitchen-sink `tsc` green, corpus back to 0 sites, and the full default Playwright project at 702 passed with one macOS-only screenshot golden. Push to get the branch verdict. |
...
| Fixed locally, push pending | this session | Detox run `30948627664` on `a7bf975a27` is the first completed run after the native fast path batch, and the Android app build failed, so `Android Detox Tests` was skipped and the branch has no Android signal at all. `:tamagui_native-registry:mergeDebugAndroidTestNativeLibs` found `lib/arm64-v8a/libc++_shared.so` twice, once from the package's own AAR and once from React Native's. The package compiles with `-DANDROID_STL=c++_shared` while excluding only `libjsi.so` and `libreact_nativemodule_core.so` from packaging. Job-by-job against pre-batch baseline run `30912958881`, this is the only job that changed direction; all four iOS Detox shards were already red at the baseline, so the batch did not break iOS. Full comparison in `plans/v3-android-verdict-a8d156b150.md`. | `packagingOptions.excludes` now carries the same seventeen-library list `react-native-nitro-modules` uses. Unverified locally: no Android build was run on this machine, so the branch Detox run is the acceptance check. |
