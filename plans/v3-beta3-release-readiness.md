...
| Blocked, external fix prepared | Nate | Exact Bento `v3` at `25af842` still has 28 callers of the removed curried `createStyledHOC(Component)(render)` signature. The production docs bundle succeeds, then static route import invokes the returned themed component outside React and fails on `useContext`. This API break was absent from the tester migration instructions; those instructions now include a before/after, and no curried caller remains in the Tamagui repository. A broken paid Bento package is also a public docs blocker because the home page imports its showcase. | Land the prepared 28-call migration on Bento `v3`, select its frozen ref, then pass the production docs build and three-mode browser test. |
