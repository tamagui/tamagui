...
| Blocked, rerun pending | a2952 | The all-package G1 preview at `b5e96fae85` stopped on publish-byte audits before isolated install. Its first run mistook an import example inside the CLI migration prompt for a runtime dependency; the audit now parses JavaScript structure and its negative/positive controls pass. Its second run found `@tamagui/tailwind` publishing `src/__tests__/candidate.test.ts` because the package included the entire `src` tree. The manifest now lists only its seven runtime source files. | After the timing quiet window, rerun the 164-package npm-pack preview through tarball audit, isolated install, web/native canary, content receipts, and beta-tag command generation. |
