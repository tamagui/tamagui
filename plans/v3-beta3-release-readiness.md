...
| Open, docs already reference it | this session | The v3 docs and the v3 blog post now tell users to `npm install --save-dev @tamagui/lsp`, and `@tamagui/lsp` has never been published. Nor have its eight per-platform binary packages, which are what actually carry the executable. If v3 ships before they do, the flat-values guide's Editor Support section and the blog's language-server section both point at a 404, and the VS Code extension has no binary to spawn. This is ordering, not engineering: the leaves must publish BEFORE the umbrella, because the umbrella pins them as exact-version `optionalDependencies` and npm treats a missing optional dependency as success, so the failure mode is a silent install with no executable rather than an error. | Not closed, but the build gate is now cleared. READ on run `31907437320` at `c69fb1a51b`, the first-ever `LSP Binaries` run: all eight targets green with `fail-fast: false`, so no target was hidden behind another's failure. The three natively-runnable targets (`aarch64-apple-darwin`, `x86_64-unknown-linux-gnu`, `x86_64-pc-windows-msvc`) each ran the handshake step and asserted the server advertises `"textDocumentSync":2`, so on three different operating systems the binary is proven to start and speak LSP rather than merely to link. The other five are cross-built and architecture-checked only, which is what a runner can honestly verify. What remains is purely the publish, and it needs the owner's explicit permission: the eight leaves first, then the umbrella, because npm treats a missing optional dependency as success. `bun run check:lsp-pins` guards the version pins and is wired into `bun run check`; its negative control is verified (exit 1 naming the stale pin). Until published, everything works from a source build via `TAMAGUI_LSP_BINARY`. |
...
The Remix and blank registry fixtures contain real build or interaction commands. The
frozen starter tag and shared release-ref setting deliver the Remix files to generated
tester projects.
...
- [x] Create annotated tag `starter/v3-beta.3` at validated candidate
  ...
  setting at it, and prove a complete generated Remix starter lands on that exact tag.
